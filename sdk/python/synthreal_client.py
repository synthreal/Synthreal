"""
Synthreal Python Client — REST API client for controlling Synthreal remotely.

Usage:

    from synthreal_client import Synthreal

    client = Synthreal("http://localhost:3000")
...
Note: This is the REST API *client* library.
      For writing Python agents that run inside Synthreal, see synthreal_sdk.py instead.
"""
...
class SynthrealError(Exception):
...
class _Resource:
    def __init__(self, client: "Synthreal"):
        self._c = client


class Synthreal:
    """Synthreal REST API client. Zero dependencies — uses only stdlib urllib."""
    ...
    def _request(self, method: str, path: str, body: Any = None) -> Any:
        ...
        try:
            ...
            raise SynthrealError(f"HTTP {e.code}: {body_text}", e.code, body_text) from e
