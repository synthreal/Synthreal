/**
 * @synthreal/sdk — Official JavaScript client for the Synthreal Agent OS REST API.
 *
 * Usage:
 *   const { Synthreal } = require("@synthreal/sdk");
 *   const client = new Synthreal("http://localhost:3000");
 *
...
class SynthrealError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "SynthrealError";
    this.status = status;
    this.body = body;
  }
}
...
class Synthreal {
  /**
   * @param {string} baseUrl - Synthreal server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new SynthrealError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new SynthrealError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { Synthreal: Synthreal, SynthrealError: SynthrealError };
