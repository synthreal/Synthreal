#!/usr/bin/env bash
# Synthreal installer — works on Linux, macOS, WSL
# Usage: curl -sSf https://synthreal.sh | sh
#
# Environment variables:
#   SYNTHREAL_INSTALL_DIR  — custom install directory (default: ~/.synthreal/bin)
#   SYNTHREAL_VERSION      — install a specific version tag (default: latest)

set -euo pipefail

REPO="synthreal/Synthreal"
INSTALL_DIR="${SYNTHREAL_INSTALL_DIR:-$HOME/.synthreal/bin}"
...
detect_platform() {
    ...
    case "$OS" in
        ...
        mingw*|msys*|cygwin*)
            ...
            echo "    irm https://synthreal.sh/install.ps1 | iex"
            echo ""
            echo "  Or download the .msi installer from:"
            echo "    https://github.com/$REPO/releases/latest"
            echo ""
            echo "  Or install via cargo:"
            echo "    cargo install --git https://github.com/$REPO synthreal-cli"
            exit 1
            ;;
...
install() {
    ...
    echo ""
    echo "  Synthreal Installer"
    echo "  =================="
    echo ""

    # Get latest version with binary assets
    if [ -n "${SYNTHREAL_VERSION:-}" ]; then
        VERSION="$SYNTHREAL_VERSION"
    ...
    URL="https://github.com/$REPO/releases/download/$VERSION/synthreal-$PLATFORM.tar.gz"
    CHECKSUM_URL="$URL.sha256"

    echo "  Installing Synthreal $VERSION for $PLATFORM..."
    mkdir -p "$INSTALL_DIR"

    # Download to temp
    TMPDIR=$(mktemp -d)
    ARCHIVE="$TMPDIR/synthreal.tar.gz"
    ...
    if [ "$OS" = "darwin" ]; then
        if command -v xattr &>/dev/null; then
            xattr -cr "$INSTALL_DIR/synthreal" 2>/dev/null || true
        fi
        if command -v codesign &>/dev/null; then
            if ! codesign --force --sign - "$INSTALL_DIR/synthreal"; then
                echo ""
                echo "  Warning: ad-hoc code signing failed."
                echo "  On Apple Silicon, the binary may be killed (SIGKILL) by Gatekeeper."
                echo "  Try manually: xattr -cr $INSTALL_DIR/synthreal && codesign --force --sign - $INSTALL_DIR/synthreal"
                echo ""
            fi
        fi
    fi
    ...
    # Fish shell: write to ~/.config/fish/conf.d/synthreal.fish (drop-in dir).
    ...
    if [ "$USE_FISH_DROPIN" -eq 1 ]; then
        ...
        FISH_DROPIN="$FISH_CONF_DIR/synthreal.fish"
        ...
        if [ ! -f "$FISH_DROPIN" ]; then
            ...
            cat > "$FISH_DROPIN" <<EOF
# Synthreal PATH — auto-generated by installer
...
EOF
        ...
        # written by older Synthreal installers (<v0.5.0). Harmless if absent.
        OLD_FISH_RC="$HOME/.config/fish/config.fish"
        if [ -f "$OLD_FISH_RC" ] && grep -q "synthreal/bin" "$OLD_FISH_RC" 2>/dev/null; then
            # Remove any line containing .synthreal/bin (covers both bash
            # `export PATH=` syntax and old fish `set -gx PATH` lines).
            TMPFILE=$(mktemp)
            grep -v "synthreal/bin" "$OLD_FISH_RC" > "$TMPFILE" || true
            mv "$TMPFILE" "$OLD_FISH_RC"
            echo "  Cleaned legacy synthreal PATH entry from $OLD_FISH_RC"
        fi
    else
        SHELL_RC=""
        case "$USER_SHELL" in
        ...
        if [ -n "$SHELL_RC" ] && ! grep -q "synthreal" "$SHELL_RC" 2>/dev/null; then
    ...
    # Verify installation
    if "$INSTALL_DIR/synthreal" --version >/dev/null 2>&1; then
        INSTALLED_VERSION=$("$INSTALL_DIR/synthreal" --version 2>/dev/null || echo "$VERSION")
        echo ""
        echo "  Synthreal installed successfully! ($INSTALLED_VERSION)"
    else
        echo ""
        echo "  Synthreal binary installed to $INSTALL_DIR/synthreal"
    fi
