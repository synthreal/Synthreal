...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg SYNTHREAL_SMOKE_FULL=1 .
...
ARG SYNTHREAL_SMOKE_FULL=0
RUN if [ "$SYNTHREAL_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$SYNTHREAL_SMOKE_FULL" = "1" ] && [ -f "$HOME/.synthreal/bin/synthreal" ]; then \
        $HOME/.synthreal/bin/synthreal --version && \
        echo "PASS: synthreal binary works"; \
    else \
