...
All notable changes to Synthreal will be documented in this file.
...
- Non-loopback requests with no `api_key` configured now return 401 by default. Opt out with `SYNTHREAL_ALLOW_NO_AUTH=1`. Fixes the B1/B2 authentication bypass from #1034.
...
- `synthreal config get default_model.base_url` now prints the configured URL instead of an empty string. Missing keys return a clear "not found" error. Fixes #905.
...
- `synthreal hand config <id>` subcommand: get, set, unset, and list settings on an active hand instance. Fixes #809.
...
- **BREAKING:** Dashboard password hashing switched from SHA256 to Argon2id. Existing `password_hash` values in `config.toml` must be regenerated with `synthreal auth hash-password`. Only affects users with `[auth] enabled = true`.
...
#### Client SDKs
- JavaScript SDK (`@synthreal/sdk`): full REST API client with streaming, TypeScript declarations
- Python client SDK (`synthreal_client`): zero-dependency stdlib client with SSE streaming
- Python agent SDK (`synthreal_sdk`): decorator-based framework for writing Python agents
...
- SynthHub marketplace with search/install
...
- MCP server (exposes Synthreal tools via MCP protocol)
...
[0.1.0]: https://github.com/synthreal/Synthreal/releases/tag/v0.1.0
