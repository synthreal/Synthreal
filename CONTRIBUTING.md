# Contributing to Synthreal

Thank you for your interest in contributing to Synthreal. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
git clone https://github.com/synthreal/Synthreal.git
cd synthreal
cargo build
```
...
```bash
cargo build --profile release-fast -p synthreal-cli
```
...
```bash
cargo test -p synthreal-kernel
cargo test -p synthreal-runtime
cargo test -p synthreal-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `SynthrealKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `synthreal-{name}` (kebab-case)
...
Synthreal is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `synthreal-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `synthreal-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `synthreal-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol |
| `synthreal-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `synthreal-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `synthreal-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `synthreal-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `synthreal-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `synthreal-wire` | OFP (Synthreal Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `synthreal-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `synthreal-migrate` | Migration engine for importing from OpenClaw (and future frameworks) |
| `synthreal-skills` | Skill system: 60 bundled skills, SynthHub marketplace, OpenClaw compatibility, prompt injection scanning |
| `synthreal-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `synthreal-runtime`, implemented on `SynthrealKernel` in `synthreal-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.synthreal/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
```bash
synthreal agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/synthreal-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/synthreal-channels/src/myplatform.rs`
...
3. Register the module in `crates/synthreal-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/synthreal-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `synthreal-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/synthreal-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/synthreal-runtime/src/tool_runner.rs`.
...
- Open a [GitHub Discussion](https://github.com/synthreal/Synthreal/discussions) for questions.
- Open a [GitHub Issue](https://github.com/synthreal/Synthreal/issues) for bugs or feature requests.
