...
const SYNTHREAL_URL = (process.env.SYNTHREAL_URL || 'http://127.0.0.1:4200').replace(/\/+$/, '');
const DEFAULT_AGENT = process.env.SYNTHREAL_DEFAULT_AGENT || 'assistant';
...
async function startConnection() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Synthreal', 'Desktop', '1.0.0'],
  });
  ...
  // Incoming messages → forward to Synthreal
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to Synthreal agent
      try {
        const response = await forwardToSynthreal(text, phone, pushName, metadata);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to Synthreal API, return agent response
// ---------------------------------------------------------------------------
function forwardToSynthreal(text, phone, pushName, metadata) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${SYNTHREAL_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Synthreal API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by Synthreal for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] Synthreal URL: ${SYNTHREAL_URL}`);
