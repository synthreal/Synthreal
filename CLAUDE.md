# Synthreal — Agent Instructions

## Project Overview
Synthreal is an open-source Agent Operating System written in Rust (14 crates).
- Config: `~/.synthreal/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/synthreal.exe` (or `target/debug/synthreal.exe`)
...
```bash
GROQ_API_KEY=<key> target/release/synthreal.exe start &
...
## Architecture Notes
- **Don't touch `synthreal-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `synthreal.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
