...
RUN cargo build --release --bin synthreal
...
COPY --from=builder /build/target/release/synthreal /usr/local/bin/
COPY --from=builder /build/agents /opt/synthreal/agents
EXPOSE 4200
VOLUME /data
ENV SYNTHREAL_HOME=/data
ENTRYPOINT ["synthreal"]
CMD ["start"]
