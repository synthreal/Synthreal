...
Synthreal is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
synthreal-cli            CLI interface, daemon auto-detect, MCP server
    |
synthreal-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
synthreal-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
synthreal-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- synthreal-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- synthreal-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- synthreal-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- synthreal-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- synthreal-skills     60 bundled skills, SynthHub marketplace, ClawHub client
    |
synthreal-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
synthreal-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `SynthrealKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.synthreal/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.synthreal/data/ exists

3. Initialize memory substrate
   - Open SQLite database (synthreal.db)
...
The memory substrate (`synthreal-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`synthreal-runtime`) provides a unified interface for all LLM providers:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, SynthrealError>;
...
The `ModelCatalog` (`synthreal-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
Synthreal implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `synthreal-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`synthreal-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`synthreal-skills`) provides 60 bundled skills and supports external skill installation.
...
- **SynthHub**: Native Synthreal marketplace (`SynthHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-Synthreal tool name mappings in `tool_compat.rs`.
...
Synthreal implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes Synthreal's 23 built-in tools via the MCP protocol. Enables external tools to use Synthreal as a tool provider.
...
The Synthreal Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`synthreal-desktop`) wraps the full Synthreal stack in a native Tauri 2.0 application.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| synthreal-runtime |  | synthreal-    |  | open-  |  | synthreal- |
|                  |  | channels     |  | synth- |  | skills    |
| +------------+   |  |              |  | wire   |  |           |
| | Agent Loop |   |  | +----------+|  |        |  | +-------+ |
