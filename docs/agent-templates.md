...
Synthreal ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
synthreal spawn orchestrator
synthreal spawn coder
synthreal spawn --template agents/writer/agent.toml
```
...
```bash
synthreal spawn security-auditor
...
```bash
synthreal spawn data-scientist
...
```bash
synthreal spawn legal-assistant
...
> General-purpose assistant. The default Synthreal agent for everyday tasks, questions, and conversations.
...
```bash
synthreal spawn email-assistant
...
```bash
synthreal spawn customer-support
...
```bash
synthreal spawn sales-assistant
...
```bash
synthreal spawn meeting-assistant
...
```bash
synthreal spawn health-tracker
...
```bash
synthreal spawn personal-finance
...
```bash
synthreal spawn travel-planner
...
```bash
synthreal spawn home-automation
...
synthreal spawn coder --name "backend-coder"

# Spawn from a TOML file path
synthreal spawn --template agents/custom/my-agent.toml

# List running agents
synthreal agents

# Send a message
synthreal message <agent-id> "Write a function to parse TOML files"

# Kill an agent
synthreal kill <agent-id>
```
