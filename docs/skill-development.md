...
Skills are pluggable tool bundles that extend agent capabilities in Synthreal. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to SynthHub, and CLI management.
...
- [Publishing to SynthHub](#publishing-to-synthhub)
...
Skills are installed to `~/.synthreal/skills/` and made available to agents through the skill registry. Synthreal ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
Synthreal includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
| `tags` | array | No | Tags for discovery on SynthHub |
...
1. Synthreal sends a JSON payload to the script's stdin:
...
```python
#!/usr/bin/env python3
"""Synthreal skill: web-summarizer"""
import json
import sys
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Synthreal-Skill/1.0"})
...
### Using the Synthreal Python SDK

For more advanced skills, use the Python SDK (`sdk/python/synthreal_sdk.py`):

```python
#!/usr/bin/env python3
from synthreal_sdk import SkillHandler
...
```bash
synthreal skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.synthreal/skills/my-skill/`.

### From SynthHub

```bash
synthreal skill install web-summarizer
```

This downloads the skill from the SynthHub marketplace registry.

### From a Git Repository

```bash
synthreal skill install https://github.com/user/synthreal-skill-example.git
```
...
```bash
synthreal skill remove web-summarizer
```

---

## Publishing to SynthHub

SynthHub is the community skill marketplace for Synthreal.
...
```bash
synthreal skill search "web scraping"
```
...
Skills matching "web scraping":
  ...
  web-summarizer (42 stars)
    ...
    https://synthhub.dev/skills/web-summarizer

  page-scraper (28 stars)
    Extract structured data from web pages
    https://synthhub.dev/skills/page-scraper
```

### Publishing

Publishing to SynthHub will be available via:

```bash
synthreal skill publish
```

This validates the manifest, packages the skill, and uploads it to the SynthHub registry.
...
```bash
# Install a skill (local directory, SynthHub name, or git URL)
...
Synthreal can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
synthreal skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), Synthreal:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to Synthreal conventions.
4. Copies the skill to the Synthreal skills directory.
...
```bash
synthreal skill install /path/to/skill-directory
```

Skills imported via `synthreal migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.
