# Synthreal Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the Synthreal Agent OS.
...
Synthreal reads its configuration from a single TOML file:

```
~/.synthreal/config.toml
```
...
1. `SYNTHREAL_HOME` environment variable (e.g. `SYNTHREAL_HOME=/data` in the official Docker image).
2. `~/.synthreal` (default).

So inside the Docker container the config file must live at `/data/config.toml` (because the image sets `ENV SYNTHREAL_HOME=/data`). Placing it anywhere else (for example `/opt/synthreal/config.toml`) will be silently ignored.
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, Synthreal boots with Anthropic as the default provider:
...
```toml
# ============================================================
# Synthreal Agent OS -- Complete Configuration Reference
# ============================================================

# --- Top-level fields ---
home_dir = "~/.synthreal"             # Synthreal home directory
data_dir = "~/.synthreal/data"        # SQLite databases and data files
...
# --- Memory ---
[memory]
# sqlite_path = "~/.synthreal/data/synthreal.db"  # Auto-resolved if omitted
...
|-------|------|---------|-------------|
| `home_dir` | path | `~/.synthreal` | Synthreal home directory. Stores config, agents, skills. |
| `data_dir` | path | `~/.synthreal/data` | Directory for SQLite databases and persistent data. |
| `log_level` | string | `"info"` | Log verbosity. One of: `trace`, `debug`, `info`, `warn`, `error`. |
| `api_listen` | string | `"127.0.0.1:50051"` | Bind address for the HTTP/WebSocket/SSE API server. Use `0.0.0.0:<port>` to accept connections from outside the host (LAN, Docker, remote clients). See [Exposing the Dashboard](#exposing-the-dashboard) below before doing so. Can be overridden at runtime with the `SYNTHREAL_LISTEN` environment variable. |
...
By default Synthreal binds the API and dashboard to `127.0.0.1` (loopback only) so the daemon is unreachable from anywhere except the local machine. To accept connections from your LAN, Docker host, or a remote client you must explicitly opt in to non-loopback binding.
...
2. Or set the `SYNTHREAL_LISTEN` environment variable (overrides `config.toml`):

   ```bash
   export SYNTHREAL_LISTEN=0.0.0.0:4200
   ```

   The env var route is the recommended path for Docker because it does not require mounting a config file.

**Docker example.** The official image sets `SYNTHREAL_HOME=/data` and exposes port `4200`. The simplest end-to-end setup is:

```yaml
services:
  synthreal:
    image: ghcr.io/synthreal/synthreal:latest
    ports:
      - "4200:4200"
    volumes:
      - synthreal-data:/data
    environment:
      - SYNTHREAL_LISTEN=0.0.0.0:4200          # required: bind to all interfaces inside the container
      - SYNTHREAL_API_KEY=${SYNTHREAL_API_KEY}  # strongly recommended when exposing
      - ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY:-}
volumes:
  synthreal-data:
```

If you prefer mounting `config.toml`, the file must live at `/data/config.toml` inside the container (because of `SYNTHREAL_HOME=/data`). Placing it at `/opt/synthreal/config.toml` or any other path will not be picked up. The port in `api_listen` must also match the port published in `ports:` — the example config in `synthreal.toml.example` ships with port `50051` to be safe; change it to `4200` (or whatever port you publish) when running in Docker.

**Security warning.** Once you bind to a non-loopback address, anyone reachable at that address can talk to the API. Synthreal's middleware enforces a fail-closed default on authenticated routes:
...
- To run with full open access anyway (not recommended), set `SYNTHREAL_ALLOW_NO_AUTH=1`. This will be loudly logged.

The supported ways to expose the dashboard safely:

- Set `api_key = "..."` in `config.toml` (or `SYNTHREAL_API_KEY=...`) and send `Authorization: Bearer <key>` on every request.
...
```toml
[memory]
# sqlite_path = "/custom/path/synthreal.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/synthreal.db`. |
...
Configures the OFP (Synthreal Protocol) peer-to-peer networking layer with HMAC-SHA256 mutual authentication.
...
enabled = true
username = "admin"
password_hash = "$argon2id$v=19$m=19456,t=2,p=1$..."  # generate with: synthreal auth hash-password
...
| `password_hash` | string | `""` (empty) | Argon2id password hash in PHC string format. Generate with `synthreal auth hash-password`. |
| `session_ttl_hours` | u64 | `168` (7 days) | Session token lifetime in hours. |

**Generating a password hash:**

```bash
synthreal auth hash-password
```

This prompts for a password and outputs an Argon2id PHC string to paste into `config.toml`.

> **Breaking change (v0.5.0):** Password hashes must be in Argon2id format. Older SHA256 hex hashes from versions prior to v0.5.0 are no longer accepted. Re-run `synthreal auth hash-password` to generate a new hash.
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@synthreal:matrix.org"`). |
...
| `nick` | string | `"synthreal"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#synthreal", "#general"]`). |
...
| `nick` | string | `"synthreal"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"synthreal"` | Bot username in Mumble. |
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across Synthreal instances.
