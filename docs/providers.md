...
Synthreal auto-detects which providers have API keys configured at boot. Any model whose provider is authenticated becomes immediately available. Local providers (Ollama, vLLM, LM Studio) require no key at all.
...
**Notes:** Synthreal auto-discovers models from a running Ollama instance and merges them into the catalog with `Local` tier and zero cost. Any model you pull becomes usable immediately.
...
Synthreal can automatically select the cheapest model capable of handling each query. This is configured per-agent via `ModelRoutingConfig`.
...
Synthreal tracks the cost of every LLM call and can enforce per-agent spending quotas.
