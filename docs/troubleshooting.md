...
Common issues, diagnostics, and answers to frequently asked questions about Synthreal.
...
Synthreal uses `tracing` for structured logging. Set the log level via environment:

```bash
RUST_LOG=info synthreal start          # Default
RUST_LOG=debug synthreal start         # Verbose
RUST_LOG=synthreal=debug synthreal start  # Only Synthreal debug, deps at info
```
...
### `synthreal` command not found after install
...
**Cause**: Older Synthreal installers (`<v0.6.4`) appended a PATH line directly to `~/.config/fish/config.fish`. On Arch derivatives like CachyOS, the desktop session can source fish on login — a malformed or invalid PATH line then prevents the session from finishing, leaving you on a black screen.

**Fix**: Boot to a TTY (`Ctrl+Alt+F2`) and remove any Synthreal PATH lines from `config.fish`:
```bash
sed -i '/synthreal/d' ~/.config/fish/config.fish
```
Then re-run the installer — current versions write to `~/.config/fish/conf.d/synthreal.fish` (a drop-in directory) instead, and guard the path with `test -d` so a missing install dir can never wedge fish startup.

To remove Synthreal's PATH entry cleanly:
```bash
rm ~/.config/fish/conf.d/synthreal.fish
```

### Docker container won't start

**Common causes**:
- No API key provided: `docker run -e GROQ_API_KEY=... ghcr.io/synthreal/Synthreal`
...
If you run Synthreal inside Docker and need to reach a service running on the
...
docker run --rm \
  ...
  -p 4200:4200 \
  ghcr.io/synthreal/synthreal:latest
```
...
The default `ghcr.io/synthreal/synthreal` image does not ship `curl`, so
`docker exec synthreal curl ...` returns `exec: curl: not found`. If you need
in-container probes for healthchecks or egress verification, build a thin
overlay image:

```dockerfile
# Dockerfile.curl
FROM ghcr.io/synthreal/synthreal:latest
USER root
RUN apt-get update \
...
```bash
docker build -f Dockerfile.curl -t synthreal-curl:latest .
docker run --rm synthreal-curl:latest curl -s https://example.com
```
...
**Fix**: Run `synthreal init` to create the default config:
```bash
synthreal init
```

This creates `~/.synthreal/config.toml` with sensible defaults.
...
```bash
RUST_LOG=synthreal_channels=debug synthreal start
```
...
**Automatic protection**: Synthreal has a built-in loop guard:
...
**Check**:
1. TOML manifest is valid: `synthreal agent spawn --dry-run manifest.toml`
...
2. Set the model to `synthreal:agent-name` (e.g., `synthreal:coder`)
...
3. Try deleting `~/.synthreal/daemon.json` and restarting
...
If slower:
- Check database size (`~/.synthreal/data/synthreal.db`)
...
1. Add the channel config to `~/.synthreal/config.toml` under `[channels]`
2. Set the required environment variables (tokens, secrets)
3. Restart the daemon

### How do I update Synthreal?

```bash
# From source
cd synthreal && git pull && cargo install --path crates/synthreal-cli

# Docker
docker pull ghcr.io/synthreal/Synthreal:latest
```
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.synthreal/data/synthreal.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.synthreal/config.toml` (configuration)
- `~/.synthreal/data/synthreal.db` (all agent data, memory, sessions)
- `~/.synthreal/skills/` (installed skills)

### How do I reset everything?

```bash
rm -rf ~/.synthreal
synthreal init  # Start fresh
```

### Can I run Synthreal without an internet connection?
...
### What's the difference between Synthreal and OpenClaw?

| Aspect | Synthreal | OpenClaw |
|--------|----------|----------|
...
Synthreal can import OpenClaw configs: `synthreal migrate --from openclaw`
...
```bash
RUST_LOG=synthreal_runtime=debug,synthreal_channels=info synthreal start
```

### Can I use Synthreal as a library?

Yes. Each crate is independently usable:
```toml
[dependencies]
synthreal-runtime = { path = "crates/synthreal-runtime" }
synthreal-memory = { path = "crates/synthreal-memory" }
```

The `synthreal-kernel` crate assembles everything, but you can use individual crates for custom integrations.
...
```bash
curl -fsSL https://synthreal.sh/install | sh
```
...
### How do I run Synthreal in Docker?

```bash
docker run -d --name synthreal \
...
Synthreal has built-in dashboard authentication. Enable it in `~/.synthreal/config.toml`:
...
Not yet — each channel type currently supports one bot. Multi-bot routing is tracked as a feature request (#586). As a workaround, run multiple Synthreal instances on different ports with different configs.

### Claude Code integration shows errors

Add to `~/.synthreal/config.toml`:
```toml
[claude_code]
