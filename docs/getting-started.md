# Getting Started with Synthreal

This guide walks you through installing Synthreal, configuring your first LLM provider, spawning an agent, and chatting with it.
...
Download the installer for your platform from the [latest release](https://github.com/synthreal/Synthreal/releases/latest):
...
The desktop app includes the full Synthreal system with a native window, system tray, auto-updates, and OS notifications. Updates are installed automatically in the background.

### Option 2: Shell Installer (Linux / macOS)

```bash
curl -sSf https://synthreal.sh | sh
```

This downloads the latest CLI binary and installs it to `~/.synthreal/bin/`.

### Option 3: PowerShell Installer (Windows)

```powershell
irm https://synthreal.sh/install.ps1 | iex
```
...
```bash
cargo install --git https://github.com/synthreal/Synthreal synthreal-cli
```
...
Run the init command to create the `~/.synthreal/` directory and a default config file:
...
Synthreal needs at least one LLM provider API key. Set it as an environment variable:
...
The default config uses Anthropic. To change the provider, edit `~/.synthreal/config.toml`:
...
Synthreal ships with 30 agent templates. Spawn the hello-world agent:

```bash
synthreal agent spawn agents/hello-world/agent.toml
```
...
```bash
synthreal agent spawn my-agent.toml
```
...
```bash
synthreal agent chat a1b2c3d4-e5f6-...
```
...
agent> I'm the hello-world agent running on Synthreal. I can:
...
```
Starting Synthreal daemon...
Synthreal daemon running on http://127.0.0.1:4200
...
Now that you have Synthreal running:
...
- **Set up workflows**: Chain multiple agents together. Use `synthreal workflow create` with a TOML workflow definition.
- **Use MCP**: Connect to external tools via Model Context Protocol. Configure in `config.toml` under `[[mcp_servers]]`.
- **Migrate from OpenClaw**: Run `synthreal migrate --from openclaw`. See [MIGRATION.md](../MIGRATION.md).
- **Desktop app**: Run `cargo tauri dev` for a native desktop experience with system tray.
- **Run diagnostics**: `synthreal doctor` checks your entire setup.

### Useful Commands Reference

```bash
synthreal init                          # Initialize ~/.synthreal/
synthreal start                         # Start the daemon
synthreal status                        # Check daemon status
synthreal doctor                        # Run diagnostic checks

synthreal agent spawn <manifest.toml>   # Spawn an agent
synthreal agent list                    # List all agents
synthreal agent chat <id>               # Chat with an agent
synthreal agent kill <id>               # Kill an agent

synthreal workflow list                 # List workflows
synthreal workflow create <file.json>   # Create a workflow
synthreal workflow run <id> <input>     # Run a workflow

synthreal trigger list                  # List event triggers
synthreal trigger create <args>         # Create a trigger
synthreal trigger delete <id>           # Delete a trigger

synthreal skill install <source>        # Install a skill
synthreal skill list                    # List installed skills
synthreal skill search <query>          # Search SynthHub
synthreal skill create                  # Scaffold a new skill

synthreal channel list                  # List channel status
synthreal channel setup <channel>       # Interactive setup wizard

synthreal config show                   # Show current config
synthreal config edit                   # Open config in editor

synthreal chat [agent]                  # Quick chat (alias)
synthreal migrate --from openclaw       # Migrate from OpenClaw
synthreal mcp                           # Start MCP server (stdio)
```
