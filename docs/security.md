# Synthreal Security Architecture

> **Security Contact:** nickhalve98@gmail.com — Report vulnerabilities via email. We respond within 48 hours.

This document provides a comprehensive technical reference for every security
system in the Synthreal Agent Operating System.  All struct names, function
...
Synthreal implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `synthreal-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `synthreal-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `synthreal-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `synthreal-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `synthreal-types` | Supply chain attacks |
| 6 | SSRF Protection | `synthreal-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `synthreal-runtime`, `synthreal-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `synthreal-wire` | Unauthorized peer connections |
| 9 | Security Headers | `synthreal-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `synthreal-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `synthreal-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `synthreal-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `synthreal-skills` | Malicious skill prompts |
| 14 | Loop Guard | `synthreal-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `synthreal-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `synthreal-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `synthreal-types/src/capability.rs`

Synthreal uses capability-based security.  An agent can only perform actions
...
**Source:** `synthreal-runtime/src/sandbox.rs`
...
**Source:** `synthreal-runtime/src/audit.rs`
...
**Source:** `synthreal-types/src/taint.rs`

Synthreal implements a lattice-based taint propagation model that prevents
...
**Source:** `synthreal-types/src/manifest_signing.rs`
...
**Source:** `synthreal-runtime/src/host_functions.rs`
...
Synthreal uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`synthreal-runtime/src/drivers/`):
...
**Channel Adapters** (`synthreal-channels/src/`):
...
**Web Search** (`synthreal-runtime/src/web_search.rs`):
...
**Embedding** (`synthreal-runtime/src/embedding.rs`):
...
**Source:** `synthreal-wire/src/peer.rs`

The Synthreal Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `synthreal-api/src/middleware.rs`
...
**Source:** `synthreal-api/src/rate_limiter.rs`

Synthreal uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `synthreal-runtime/src/subprocess_sandbox.rs`
...
**Source:** `synthreal-skills/src/verify.rs`
...
**Source:** `synthreal-runtime/src/loop_guard.rs`
...
**Source:** `synthreal-runtime/src/session_repair.rs`
...
**Source:** `synthreal-api/src/routes.rs`

Synthreal provides two health endpoints with different information levels.
