...
cargo tauri signer generate -w ~/.tauri/synthreal.key
```
...
Your private key was saved to: ~/.tauri/synthreal.key
```
...
Open `crates/synthreal-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/synthreal.key` | Yes |
...
The following icon files must exist in `crates/synthreal-desktop/icons/`:
...
## 5. Set Up the `synthreal.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://synthreal.sh | sh`.

Options:
- **GitHub Pages**: Point `synthreal.sh` to a GitHub Pages site that redirects `/` to `scripts/install.sh` and `/install.ps1` to `scripts/install.ps1` from the repo's latest release.
- **Cloudflare Workers / Vercel**: Serve the install scripts with proper `Content-Type: text/plain` headers.
- **Raw GitHub redirect**: Use `synthreal.sh` as a CNAME to `raw.githubusercontent.com/synthreal/Synthreal/main/scripts/install.sh` (less reliable).

The install scripts reference:
- `https://synthreal.sh` → serves `scripts/install.sh`
- `https://synthreal.sh/install.ps1` → serves `scripts/install.ps1`

Until the domain is set up, users can install via:
```bash
curl -sSf https://raw.githubusercontent.com/synthreal/Synthreal/main/scripts/install.sh | sh
```
...
```bash
docker build -t synthreal:local .
docker run --rm synthreal:local --version
docker run --rm -p 4200:4200 -v synthreal-data:/data synthreal:local start
```
...
grep '"version"' crates/synthreal-desktop/tauri.conf.json
...
Visit: `https://github.com/synthreal/Synthreal/releases/latest/download/latest.json`
...
docker pull ghcr.io/synthreal/Synthreal:0.1.0

# Verify both architectures
docker run --rm ghcr.io/synthreal/Synthreal:latest --version
```
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "Synthreal Updating..." notification
...
synthreal --version  # Should print v0.1.0
