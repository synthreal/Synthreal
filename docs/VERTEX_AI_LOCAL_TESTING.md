...
```batch
# Run this from the synthreal directory:
start-vertex.bat
```
...
taskkill /F /IM synthreal.exe 2>$null
...
# 4. Start Synthreal
cd C:\Users\at384\Downloads\osc\dllm\synthreal
.\target\debug\synthreal.exe start
```
...
### Direct Vertex AI Test (Bypass Synthreal)
...
**Cause:** Synthreal not running or wrong port.
...
cargo build -p synthreal-cli

# Run tests
cargo test -p synthreal-runtime --lib vertex

# Check formatting
cargo fmt --check -p synthreal-runtime

# Run clippy
cargo clippy -p synthreal-runtime --lib -- -W warnings
```
...
- `crates/synthreal-runtime/src/drivers/vertex.rs` (NEW - ~790 lines)
- `crates/synthreal-runtime/src/drivers/mod.rs` (+62 lines)
