...
1. **Add token estimation & context guard** (`crates/synthreal-runtime/src/compactor.rs`)
...
2. **Add in-loop token guard** (`crates/synthreal-runtime/src/agent_loop.rs`)
...
3. **Filter tools by profile in kernel** (`crates/synthreal-kernel/src/kernel.rs`)
...
4. **Raise default token quota** (`crates/synthreal-types/src/agent.rs`)
   - Change `max_llm_tokens_per_hour` from 100_000 to 1_000_000
   - 100K is too low — a single system prompt is 30-40K tokens

5. **Token-based compaction trigger** (`crates/synthreal-kernel/src/kernel.rs`)
...
6. **Compact system prompt injections** (`crates/synthreal-kernel/src/kernel.rs`)
...
**Problem:** Desktop app may show Tauri default icons. Branding assets exist at `~/Downloads/synthreal/output/` but aren't installed.

**What to do:**

1. Generate all required icon sizes from source PNG (`synthreal-logo-transparent.png`, 2000x2000)
2. Place into `crates/synthreal-desktop/icons/`:
   - `icon.png` (1024x1024)
   - `icon.ico` (multi-size: 256, 128, 64, 48, 32, 16)
   - `32x32.png`
   - `128x128.png`
   - `128x128@2x.png` (256x256)
3. Replace web UI logo at `crates/synthreal-api/static/logo.png`
4. Update favicon if one exists

**Assets available:**
- `synthreal-logo-transparent.png` (328KB, 2000x2000) — primary source
- `synthreal-logo-black-bg.png` (312KB) — for dark contexts
- `synthreal-vector-transparent.svg` (293KB) — scalable vector
- `synthreal-animated.svg` (310KB) — for loading screens

**Done when:**
- Desktop app shows Synthreal logo in taskbar, title bar, and installer
...
**Status: COMPLETE** — Generated Ed25519 signing keypair via `cargo tauri signer generate --ci`. Public key installed in `tauri.conf.json`. Private key at `~/.tauri/synthreal.key`. Set `TAURI_SIGNING_PRIVATE_KEY_PATH` in CI secrets.
...
These close the gaps that would make users pick OpenClaw over Synthreal.
...
**Files:** `crates/synthreal-api/static/js/pages/chat.js`, `crates/synthreal-runtime/src/tool_runner.rs`
...
4. Add SynthHub registry URL if not configured

**Files:** `crates/synthreal-api/static/js/pages/skills.js`, `crates/synthreal-api/src/routes.rs`
...
**Problem:** `synthreal.sh` domain isn't set up. Users can't do `curl -sSf https://synthreal.sh | sh`.

**What to do:**
1. Set up GitHub Pages or Cloudflare Worker for synthreal.sh
...
**Done when:**
- `curl -sSf https://synthreal.sh | sh` installs the latest release
- `irm https://synthreal.sh/install.ps1 | iex` works on Windows PowerShell
...
These are features where Synthreal can leapfrog OpenClaw.
...
**Status: COMPLETE** — Created `sdk/javascript/` (@synthreal/sdk) with full REST client: agent CRUD, streaming via SSE, sessions, workflows, skills, channels, memory KV, triggers, schedules + TypeScript declarations. Created `sdk/python/synthreal_client.py` (zero-dependency stdlib urllib) with same coverage. Both include basic + streaming examples. Python `setup.py` for pip install.
...
1. Create `sdks/javascript/` — thin wrapper around REST API
   ...
   - Publish to npm as `@synthreal/sdk`
2. Create `sdks/python/` — thin wrapper with httpx
   - Same operations
   - Publish to PyPI as `synthreal`
3. Include usage examples in README

**Done when:**
- `npm install @synthreal/sdk` works
- `pip install synthreal` works
...
**Status: COMPLETE** — Added `GET /api/metrics` endpoint returning Prometheus text format. Metrics: `synthreal_uptime_seconds`, `synthreal_agents_active`, `synthreal_agents_total`, `synthreal_tokens_total{agent,provider,model}`, `synthreal_tool_calls_total{agent}`, `synthreal_panics_total`, `synthreal_restarts_total`, `synthreal_info{version}`.

**Problem (was):** No way to monitor Synthreal in production (no Prometheus, no OpenTelemetry).

**What to do:**
1. Add `/api/metrics` endpoint with Prometheus format
   - `synthreal_agents_active` gauge
   - `synthreal_messages_total` counter (by agent, by channel)
   - `synthreal_tokens_total` counter (by provider, by model)
   - `synthreal_request_duration_seconds` histogram
   - `synthreal_tool_calls_total` counter (by tool name)
   - `synthreal_errors_total` counter (by type)
2. Optional: OTLP export for tracing spans

**Files:** `crates/synthreal-api/src/routes.rs`, new `metrics.rs` module
...
**Problem (was):** Both Synthreal and OpenClaw define workflows in TOML/config only. No visual builder exists in either. First to ship this wins.
...
**Files:** `crates/synthreal-kernel/src/kernel.rs`, `routes.rs`, `ws.rs`, `index_body.html`
...
**What to do:**
1. Watch `~/.synthreal/config.toml` for changes (notify crate)
2. On change: re-parse, diff, apply only changed sections
3. Log what was reloaded
4. UI notification: "Config reloaded"

**Files:** `crates/synthreal-api/src/server.rs`, `crates/synthreal-types/src/config.rs`
...
**Status: ALL CODE COMPLETE** — All 18 code items done. 1751 tests passing. Production audit completed: 2 critical bugs fixed (API delete alias, config/set route), CSP hardened (Tauri + middleware), Tauri signing key installed. Remaining for release: tag v0.1.0, build release artifacts, set up synthreal.sh domain.
...
| Feature | OpenClaw | Synthreal | Winner |
|---------|----------|----------|--------|
| Language/Performance | Node.js (~200MB) | Rust (~30MB single binary) | **Synthreal** |
| Channels | ~15 | **40** | **Synthreal** |
| Built-in Tools | ~19 | **41** | **Synthreal** |
| Security Systems | Token + sandbox | **16 defense systems** | **Synthreal** |
| Agent Templates | Manual config | **30 pre-configured** | **Synthreal** |
| Hands (autonomous) | None | **7 packages** | **Synthreal** |
| Workflow Engine | Cron + webhooks | **Full DAG with parallel/loops** | **Synthreal** |
| Knowledge Graph | Flat vector store | **Entity-relation graph** | **Synthreal** |
| P2P Networking | None | **OFP wire protocol** | **Synthreal** |
| WASM Sandbox | Docker only | **Dual-metered WASM** | **Synthreal** |
| Desktop App | Electron (~200MB) | **Tauri (~30MB)** | **Synthreal** |
| Migration | N/A | **`migrate --from openclaw`** | **Synthreal** |
| Skills | 54 bundled | **60 bundled** | **Synthreal** |
| LLM Providers | ~15 | **27 providers, 130+ models** | **Synthreal** |
...
| Visual Workflow Builder | None | **Drag-and-drop builder** | **Synthreal** |

**Synthreal wins 15/18 categories.** The remaining gaps are: mobile apps (OpenClaw), voice wake word (OpenClaw slight edge).
...
Sprint 2: 4/5 COMPLETE
  ...
  2.4 Install script domain ........ PENDING (infra: set up synthreal.sh domain)
