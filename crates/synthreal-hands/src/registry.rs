...
impl HandRegistry {
    ...
    /// install where the user has not run `synthreal hand install` yet.
    ///
    /// Added for issue #984 — custom hands installed via `synthreal hand
...
mod tests {
    ...
    fn env_var_requirement_check() {
        std::env::set_var("SYNTHREAL_TEST_HAND_REQ", "test_value");
        let req = HandRequirement {
            key: "test".to_string(),
            label: "test".to_string(),
            requirement_type: RequirementType::EnvVar,
            check_value: "SYNTHREAL_TEST_HAND_REQ".to_string(),
            description: None,
            optional: false,
            install: None,
        };
        ...
        let req_missing = HandRequirement {
            ...
            label: "test".to_string(),
            requirement_type: RequirementType::EnvVar,
            check_value: "SYNTHREAL_NONEXISTENT_VAR_12345".to_string(),
            description: None,
            optional: false,
            install: None,
        };
        assert!(!check_requirement(&req_missing));
        std::env::remove_var("SYNTHREAL_TEST_HAND_REQ");
    }
...
"#;
    ...
    ///
    /// Simulates what `synthreal hand config <id> --set KEY=VAL` does against
    ...
    fn test_hand_config_round_trip_via_registry() {
        ...
        cfg.insert(
            ...
            serde_json::Value::String("synthreal/1".into()),
        );
        ...
        assert_eq!(
            ...
            Some(&serde_json::Value::String("synthreal/1".into()))
        );
