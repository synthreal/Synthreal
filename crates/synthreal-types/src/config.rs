//! Configuration types for the Synthreal kernel.
...
impl Default for WebhookTriggerConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            token_env: "SYNTHREAL_WEBHOOK_TOKEN".to_string(),
...
pub struct FallbackProviderConfig {
    ...
    /// it today. The `SYNTHREAL_SUBPROCESS_TIMEOUT_SECS` env var, if set, wins
...
pub struct DockerSandboxConfig {
    ...
    pub image: String,
    /// Container name prefix. Default: "synthreal-sandbox".
...
impl Default for DockerSandboxConfig {
    fn default() -> Self {
        Self {
            ...
            container_prefix: "synthreal-sandbox".to_string(),
...
pub struct VaultConfig {
    ...
    pub enabled: bool,
    /// Custom vault file path (default: ~/.synthreal/vault.enc).
...
/// - `ChannelMessage::channel_id()` in `synthreal-channels::types` (routing-time
...
pub struct ExecPolicy {
    ...
    /// Environment variables to forward from the Synthreal process into
...
pub struct KernelConfig {
    /// Synthreal home directory (default: ~/.synthreal).
    pub home_dir: PathBuf,
    /// Data directory for databases (default: ~/.synthreal/data).
    ...
    /// Root directory for agent workspaces. Default: `~/.synthreal/workspaces`
    #[serde(default)]
    ...
    /// Defaults to `~/.synthreal/workflows`. Set to empty string to disable.
    #[serde(default)]
...
pub struct AuthConfig {
    ...
    /// Generate with: synthreal auth hash-password
...
impl Default for KernelConfig {
    fn default() -> Self {
        let home_dir = synthreal_home_dir();
        Self {
...
/// Resolve the Synthreal home directory.
///
/// Priority: `SYNTHREAL_HOME` env var > `~/.synthreal`.
fn synthreal_home_dir() -> PathBuf {
...
pub struct DefaultModelConfig {
    ...
    /// `SYNTHREAL_SUBPROCESS_TIMEOUT_SECS` env var, if set, wins over this
...
pub struct MatrixConfig {
    ...
    /// Bot user ID (e.g., "@synthreal:matrix.org").
    pub user_id: String,
...
pub struct IrcConfig {
    ...
    /// Channels to join (e.g., `["#synthreal", "#general"]`).
    #[serde(default, deserialize_with = "deserialize_string_or_int_vec")]
...
impl Default for MqttConfig {
    fn default() -> Self {
        Self {
            ...
            client_id: String::new(),
            subscribe_topic: "synthreal/inbox".to_string(),
...
mod tests {
    ...
    fn test_validate_missing_env_vars() {
        ...
        config.channels.discord = Some(DiscordConfig {
            bot_token_env: "SYNTHREAL_TEST_NONEXISTENT_VAR_DC".to_string(),
            ..Default::default()
        });
        let warnings = config.validate();
