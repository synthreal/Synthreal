...
impl MigrationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str("1. Review imported agent manifests in `~/.synthreal/agents/`\n");
        out.push_str(
            "2. Review `~/.synthreal/secrets.env` — verify tokens were migrated correctly\n",
        );
        out.push_str("3. Set any remaining API keys referenced in `~/.synthreal/config.toml`\n");
        out.push_str("4. Start the daemon: `synthreal start`\n");
        out.push_str("5. Test your agents: `synthreal agent list`\n");
    ...
    pub fn print_summary(&self) {
        ...
        if !self.dry_run {
            println!("\n  Next steps:");
            println!("    synthreal start");
            println!("    synthreal agent list");
        }
    }
}
    ...
    fn test_report_with_items() {
        let report = MigrationReport {
            ...
            imported: vec![MigrateItem {
                ...
                destination: "~/.synthreal/agents/coder/agent.toml".to_string(),
            }],
