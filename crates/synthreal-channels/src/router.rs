...
use synthreal_types::config::{AgentBinding, BroadcastConfig, BroadcastStrategy};
use std::sync::Mutex;
use tracing::warn;
...
pub struct AgentRouter {
    /// Default agent per user (keyed by synthreal_user or platform_id).
...
mod tests {
    ...
    fn test_binding_channel_match() {
        ...
        router.load_bindings(&[AgentBinding {
            ...
            match_rule: synthreal_types::config::BindingMatchRule {
    ...
    fn test_binding_specificity_scores() {
        use synthreal_types::config::BindingMatchRule;
    ...
    fn test_binding_match_rule_unknown_field_rejected() {
        ...
        let r: Result<synthreal_types::config::BindingMatchRule, _> = serde_json::from_str(bad);
        ...
        let r: synthreal_types::config::BindingMatchRule = serde_json::from_str(good).unwrap();
