...
pub struct ChannelUser {
    ...
    /// Optional mapping to an Synthreal user identity.
    pub synthreal_user: Option<String>,
}
...
// Re-export the adapter allowlist from synthreal-types so config validation
// and routing share a single source of truth (no drift between the two).
pub use synthreal_types::config::CHANNELS_WITH_PLATFORM_ID_AS_CHANNEL;
...
// Re-export policy/format types from synthreal-types for convenience.
pub use synthreal_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the Synthreal kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = synthreal_types::truncate_str(remaining, max_len).len();
