//! IRC channel adapter for the Synthreal channel bridge.
//!
...
pub struct IrcAdapter {
    ...
    /// IRC channels to join (e.g., ["#synthreal", "#bots"]).
    channels: Vec<String>,
        ...
        *self.write_tx.write().await = Some(write_cmd_tx.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                registration.push_str(&format!("USER {nick} 0 * :Synthreal Bot\r\n"));
                    ...
                    tokio::select! {
                        ...
                        _ = shutdown_rx.changed() => {
                            if *shutdown_rx.borrow() {
                                ...
                                let _ = writer.write_all(b"QUIT :Synthreal shutting down\r\n").await;
                                return;
                            }
                        }
                    }
...
mod tests {
    ...
    fn test_parse_irc_line_privmsg() {
        let line = parse_irc_line(":alice!alice@host PRIVMSG #synthreal :Hello everyone!").unwrap();
        assert_eq!(line.prefix.as_deref(), Some("alice!alice@host"));
        assert_eq!(line.command, "PRIVMSG");
        assert_eq!(line.params, vec!["#synthreal"]);
    ...
    fn test_parse_irc_line_no_trailing() {
        let line = parse_irc_line(":alice!alice@host JOIN #synthreal").unwrap();
    ...
    fn test_parse_privmsg_channel() {
        let line = IrcLine {
            ...
            params: vec!["#synthreal".to_string()],
            trailing: Some("Hello from IRC!".to_string()),
        };

        let msg = parse_privmsg(&line, "synthreal-bot").unwrap();
        assert_eq!(msg.channel, ChannelType::Custom("irc".to_string()));
        assert_eq!(msg.sender.display_name, "alice");
        assert_eq!(msg.sender.platform_id, "#synthreal");
        assert!(msg.is_group);
    ...
    fn test_parse_privmsg_dm() {
        let line = IrcLine {
            ...
            command: "PRIVMSG".to_string(),
            params: vec!["synthreal-bot".to_string()],
    ...
    fn test_parse_privmsg_skips_self() {
        let line = IrcLine {
            prefix: Some("synthreal-bot!bot@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#synthreal".to_string()],
            trailing: Some("My own message".to_string()),
        };

        let msg = parse_privmsg(&line, "synthreal-bot");
        assert!(msg.is_none());
    }
