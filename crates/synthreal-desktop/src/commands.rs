...
use synthreal_kernel::config::synthreal_home;
...
/// `~/.synthreal/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = synthreal_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.synthreal/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = synthreal_home().join("skills");
...
/// Open the Synthreal config directory (`~/.synthreal/`) in the OS file manager.
#[tauri::command]
pub fn open_config_dir() -> Result<(), String> {
    let dir = synthreal_home();
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create config dir: {e}"))?;
    open::that(&dir).map_err(|e| format!("Failed to open directory: {e}"))
}

/// Open the Synthreal logs directory (`~/.synthreal/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = synthreal_home().join("logs");
