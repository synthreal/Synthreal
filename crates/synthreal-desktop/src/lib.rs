//! Synthreal Desktop — Native Tauri 2.0 wrapper for the Synthreal Agent OS.
//!
...
use synthreal_types::event::{EventPayload, LifecycleEvent, SystemEvent};
use std::sync::Arc;
...
pub fn run() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "synthreal=info,tauri=info".into()),
        )
        .init();

    info!("Starting Synthreal Desktop...");

    // Boot kernel + embedded server (blocks until port is known)
    let server_handle = server::start_server().expect("Failed to start Synthreal server");
    let port = server_handle.port;
    let kernel_for_notifications = server_handle.kernel.clone();

    info!("Synthreal server running on port {port}");
    ...
    builder
        ...
        .setup(move |app| {
                        ...
                        Ok(event) => {
                            let (title, body) = match &event.payload {
                                ...
                                EventPayload::System(SystemEvent::KernelStopping) => (
                                    ...
                                    "Synthreal kernel is shutting down".to_string(),
                                ),
            ...
            info!("Synthreal Desktop window created");
            Ok(())
        })
