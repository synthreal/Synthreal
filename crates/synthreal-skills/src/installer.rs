...
//!
//! Wraps the per-source install clients (SynthHub `marketplace`, ClawHub) with
...
//! [`synthreal_types::manifest_signing::SignedManifest`]. The installer looks
...
use synthreal_types::manifest_signing::SignedManifest;
use std::path::Path;
...
"#;
    ...
    fn require_signed_binds_to_package_json() {
        let dir = TempDir::new().unwrap();
        let pkg = r#"{"name":"x","version":"0.1.0","synthreal":{"skill":"x"}}"#;
