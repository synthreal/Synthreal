...
//!
//! This module detects both formats and converts them to Synthreal `SkillManifest`.
...
use synthreal_types::tool_compat;
...
/// Result of converting a SKILL.md into Synthreal format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → synthreal_name).
...
/// Full conversion of a SKILL.md directory to Synthreal format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to Synthreal tool definitions
        for cmd in &meta.commands {
            ...
            let synthreal_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_synthreal_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into Synthreal format.
///
...
/// Convert an OpenClaw Node.js skill directory into an Synthreal SkillManifest.
///
...
/// Write an Synthreal skill.toml manifest for an OpenClaw skill.
pub fn write_synthreal_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
