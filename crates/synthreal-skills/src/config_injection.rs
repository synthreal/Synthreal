...
//!
//! 1. User-supplied config (e.g., `[skills.<skill-name>]` in `~/.synthreal/config.toml`)
...
///    of `~/.synthreal/config.toml`, passed in by the caller.
...
pub fn render_config_block(resolved: &HashMap<String, String>) -> String {
    ...
    let mut out = String::from("[Skill config from ~/.synthreal/config.toml:\n");
    for key in keys {
...
mod tests {
    ...
    fn resolve_env_when_no_user_config() {
        ...
        let env_name = "SYNTHREAL_TEST_CFG_ENV_7f3a";
    ...
    fn resolve_default_when_no_user_no_env() {
        ...
        vars.insert(
            "default_branch".to_string(),
            var(Some("SYNTHREAL_UNSET_9z1"), Some("main"), false),
        );
    ...
    fn resolve_missing_required_returns_error() {
        ...
        vars.insert(
            ...
            var(Some("SYNTHREAL_UNSET_a2b3"), None, true),
        );
    ...
    fn resolve_missing_non_required_is_dropped() {
        ...
        vars.insert(
            "optional".to_string(),
            var(Some("SYNTHREAL_UNSET_c4d5"), None, false),
        );
