...
use synthreal_types::memory::{MemoryFilter, MemoryFragment, MemoryId, MemorySource};
...
impl SemanticStore {
    ...
    pub fn recall_with_embedding(
        ...
        for row_result in rows {
            let (
                ...
                access_count,
                embedding_bytes,
            ) = row_result.map_err(|e| SynthrealError::Memory(e.to_string()))?;

            let id = uuid::Uuid::parse_str(&id_str)
                .map(MemoryId)
                .map_err(|e| SynthrealError::Memory(e.to_string()))?;
            let agent_id = uuid::Uuid::parse_str(&agent_str)
                .map(synthreal_types::agent::AgentId)
                .map_err(|e| SynthrealError::Memory(e.to_string()))?;
    ...
    pub fn forget(&self, id: MemoryId) -> SynthrealResult<()> {
        #[cfg(feature = "http-memory")]
    ...
    fn recall_via_http(
        ...
        let results = client
            ...
            .map_err(|e| SynthrealError::Memory(format!("HTTP search failed: {e}")))?;
