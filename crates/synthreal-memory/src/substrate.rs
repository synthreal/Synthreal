...
use synthreal_types::agent::{AgentEntry, AgentId, SessionId};
use synthreal_types::config::MemoryConfig;
use synthreal_types::error::{SynthrealError, SynthrealResult};
use synthreal_types::memory::{
...
impl MemorySubstrate {
    ...
    ) -> SynthrealResult<Self> {
        let conn = Connection::open(db_path).map_err(|e| SynthrealError::Memory(e.to_string()))?;
        conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000;")
            .map_err(|e| SynthrealError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| SynthrealError::Memory(e.to_string()))?;
    ...
    pub fn open_in_memory(decay_rate: f32) -> SynthrealResult<Self> {
        let conn =
            Connection::open_in_memory().map_err(|e| SynthrealError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| SynthrealError::Memory(e.to_string()))?;
        let shared = Arc::new(Mutex::new(conn));
    ...
    pub fn save_agent(&self, entry: &AgentEntry) -> SynthrealResult<()> {
        self.structured.save_agent(entry)
    }

    /// Load an agent entry from persistent storage.
    pub fn load_agent(&self, agent_id: AgentId) -> SynthrealResult<Option<AgentEntry>> {
        self.structured.load_agent(agent_id)
    }

    /// Remove an agent from persistent storage and cascade-delete sessions.
    pub fn remove_agent(&self, agent_id: AgentId) -> SynthrealResult<()> {
    ...
    pub fn load_all_agents(&self) -> SynthrealResult<Vec<AgentEntry>> {
        self.structured.load_all_agents()
    }

    /// List all saved agents.
    pub fn list_agents(&self) -> SynthrealResult<Vec<(String, String, String)>> {
    ...
    pub fn structured_get(
        ...
        agent_id: AgentId,
        key: &str,
    ) -> SynthrealResult<Option<serde_json::Value>> {
        self.structured.get(agent_id, key)
    }

    /// List all KV pairs for an agent.
    pub fn list_kv(&self, agent_id: AgentId) -> SynthrealResult<Vec<(String, serde_json::Value)>> {
        self.structured.list_kv(agent_id)
    }

    /// Delete a KV entry for an agent.
    pub fn structured_delete(&self, agent_id: AgentId, key: &str) -> SynthrealResult<()> {
    ...
    /// Get a session by ID.
    pub fn get_session(&self, session_id: SessionId) -> SynthrealResult<Option<Session>> {
        self.sessions.get_session(session_id)
    }

    /// Save a session.
    pub fn save_session(&self, session: &Session) -> SynthrealResult<()> {
        self.sessions.save_session(session)
    }

    /// Save a session asynchronously — runs the SQLite write in a blocking
    /// thread so the tokio runtime stays responsive.
    pub async fn save_session_async(&self, session: &Session) -> SynthrealResult<()> {
        let sessions = self.sessions.clone();
        let session = session.clone();
        tokio::task::spawn_blocking(move || sessions.save_session(&session))
            .await
            .map_err(|e| SynthrealError::Internal(e.to_string()))?
    }

    /// Create a new empty session for an agent.
    pub fn create_session(&self, agent_id: AgentId) -> SynthrealResult<Session> {
        self.sessions.create_session(agent_id)
    }

    /// List all sessions with metadata.
    pub fn list_sessions(&self) -> SynthrealResult<Vec<serde_json::Value>> {
        self.sessions.list_sessions()
    }

    /// Delete a session by ID.
    pub fn delete_session(&self, session_id: SessionId) -> SynthrealResult<()> {
        self.sessions.delete_session(session_id)
    }

    /// Delete all sessions belonging to an agent.
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> SynthrealResult<()> {
        self.sessions.delete_agent_sessions(agent_id)
    }

    /// Delete the canonical (cross-channel) session for an agent.
    pub fn delete_canonical_session(&self, agent_id: AgentId) -> SynthrealResult<()> {
    ...
    pub fn find_session_by_label(
        ...
        agent_id: AgentId,
        label: &str,
    ) -> SynthrealResult<Option<Session>> {
        self.sessions.find_session_by_label(agent_id, label)
    }

    /// List all sessions for a specific agent.
    pub fn list_agent_sessions(&self, agent_id: AgentId) -> SynthrealResult<Vec<serde_json::Value>> {
    ...
    ) -> SynthrealResult<(Option<String>, Vec<synthreal_types::message::Message>)> {
    ...
    pub fn store_llm_summary(
        ...
        agent_id: AgentId,
        summary: &str,
        kept_messages: Vec<synthreal_types::message::Message>,
    ) -> SynthrealResult<()> {
        self.sessions
    ...
    pub fn append_canonical(
        &self,
        agent_id: AgentId,
        messages: &[synthreal_types::message::Message],
    ...
    pub fn load_paired_devices(&self) -> SynthrealResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SynthrealError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| SynthrealError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> SynthrealResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| SynthrealError::Memory(e.to_string()))?;
    ...
    ) -> SynthrealResult<Vec<MemoryFragment>> {
        self.semantic
            .recall_with_embedding(query, limit, filter, query_embedding)
    }

    /// Update the embedding for an existing memory.
    pub fn update_embedding(&self, id: MemoryId, embedding: &[f32]) -> SynthrealResult<()> {
    ...
    pub async fn task_post(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let db = conn.lock().map_err(|e| SynthrealError::Internal(e.to_string()))?;
            db.execute(
    ...
    pub async fn task_claim(&self, agent_id: &str) -> SynthrealResult<Option<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Err(e) => Err(SynthrealError::Memory(e.to_string())),
            }
        })
        .await
        .map_err(|e| SynthrealError::Internal(e.to_string()))?
    ...
    pub async fn task_complete(&self, task_id: &str, result: &str) -> SynthrealResult<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if rows == 0 {
                return Err(SynthrealError::Internal(format!("Task not found: {task_id}")));
            }
            Ok(())
        })
        .await
    ...
    pub async fn task_list(&self, status: Option<&str>) -> SynthrealResult<Vec<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = db.prepare(sql).map_err(|e| SynthrealError::Memory(e.to_string()))?;
            ...
            for row in rows {
                tasks.push(row.map_err(|e| SynthrealError::Memory(e.to_string()))?);
            }
            Ok(tasks)
        })
        .await
...
impl Memory for MemorySubstrate {
    async fn get(&self, agent_id: AgentId, key: &str) -> SynthrealResult<Option<serde_json::Value>> {
    ...
    async fn delete(&self, agent_id: AgentId, key: &str) -> SynthrealResult<()> {
        let store = self.structured.clone();
        let key = key.to_string();
    ...
    async fn forget(&self, id: MemoryId) -> SynthrealResult<()> {
        let store = self.semantic.clone();
    ...
    async fn add_entity(&self, entity: Entity) -> SynthrealResult<String> {
    ...
    async fn add_relation(&self, relation: Relation) -> SynthrealResult<String> {
        let store = self.knowledge.clone();
    ...
    async fn query_graph(&self, pattern: GraphPattern) -> SynthrealResult<Vec<GraphMatch>> {
        let store = self.knowledge.clone();
    ...
    async fn consolidate(&self) -> SynthrealResult<ConsolidationReport> {
    ...
    async fn export(&self, format: ExportFormat) -> SynthrealResult<Vec<u8>> {
        let _ = format;
        Ok(Vec::new())
    }

    async fn import(&self, _data: &[u8], _format: ExportFormat) -> SynthrealResult<ImportReport> {
        Ok(ImportReport {
            entities_imported: 0,
