...
use dashmap::DashMap;
use synthreal_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> SynthrealResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(SynthrealError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> SynthrealResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: synthreal_types::agent::SessionId,
    ) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: synthreal_types::agent::AgentIdentity,
    ) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_fallback_models(
        &self,
        id: AgentId,
        fallback_models: Vec<synthreal_types::agent::FallbackModel>,
    ) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> SynthrealResult<()> {
        if let Some(existing_id) = self.name_index.get(&new_name).as_deref().copied() {
            if existing_id != id {
                return Err(SynthrealError::AgentAlreadyExists(new_name));
            }
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> SynthrealResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| SynthrealError::AgentNotFound(id.to_string()))?;
