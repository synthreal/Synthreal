...
//! to Synthreal users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use synthreal_types::agent::UserId;
use synthreal_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Synthreal user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Synthreal UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> SynthrealResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| SynthrealError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(SynthrealError::AuthDenied(format!(
