//! Configuration loading from `~/.synthreal/config.toml` with defaults.
//!
...
use synthreal_types::config::KernelConfig;
...
fn lenient_extract_bindings(root_value: &mut toml::Value) {
    use synthreal_types::config::AgentBinding;
...
///
/// Respects `SYNTHREAL_HOME` env var (e.g. `SYNTHREAL_HOME=/opt/synthreal`).
pub fn default_config_path() -> PathBuf {
    synthreal_home().join("config.toml")
}

/// Get the Synthreal home directory.
///
/// Priority: `SYNTHREAL_HOME` env var > `~/.synthreal`.
pub fn synthreal_home() -> PathBuf {
