...
use synthreal_types::model_catalog::{
...
/// (VPS, LXC, another box on the LAN) without editing `~/.synthreal/config.toml`.
///
...
pub fn create_driver(config: &DriverConfig) -> Result<Arc<dyn LlmDriver>, LlmError> {
    ...
    if provider == "claude-code" {
        ...
        //   1. SYNTHREAL_SUBPROCESS_TIMEOUT_SECS env var (no-rebuild override for emergencies)
        ...
        let timeout = std::env::var("SYNTHREAL_SUBPROCESS_TIMEOUT_SECS")
            .ok()
    ...
    // Run `synthreal config set-key github-copilot` to authenticate.
    if provider == "github-copilot" || provider == "copilot" {
        let synthreal_dir = std::env::var("HOME")
            .or_else(|_| std::env::var("USERPROFILE"))
            .map(|h| std::path::PathBuf::from(h).join(".synthreal"))
            .unwrap_or_else(|_| std::path::PathBuf::from(".synthreal"));

        if !copilot::copilot_auth_available(&synthreal_dir) {
            return Err(LlmError::MissingApiKey(
                "Copilot not authenticated. Run `synthreal config set-key github-copilot` to sign in."
                    .to_string(),
            ));
        }

        return Ok(Arc::new(copilot::CopilotDriver::new(synthreal_dir)));
    }
...
mod tests {
    ...
    fn test_claude_code_driver_constructs_with_default_timeout() {
        ...
        std::env::remove_var("SYNTHREAL_SUBPROCESS_TIMEOUT_SECS");
        let config = DriverConfig {
    ...
    fn test_claude_code_driver_constructs_with_env_timeout_override() {
        ...
        std::env::set_var("SYNTHREAL_SUBPROCESS_TIMEOUT_SECS", "600");
        let config = DriverConfig {
            provider: "claude-code".to_string(),
            api_key: None,
            base_url: None,
    ...
    fn test_claude_code_driver_ignores_unparseable_env_timeout() {
        ...
        std::env::set_var("SYNTHREAL_SUBPROCESS_TIMEOUT_SECS", "not-a-number");
        let config = DriverConfig {
            provider: "claude-code".to_string(),
            api_key: None,
            base_url: None,
