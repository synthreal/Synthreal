...
use dashmap::DashMap;
use synthreal_types::message::{ContentBlock, MessageContent, Role, StopReason, TokenUsage};
        ...
        if skip_permissions {
            warn!(
                "Claude Code driver: --dangerously-skip-permissions enabled. \
                 ...
                 Synthreal's own capability/RBAC system enforces access control."
            );
        }
...
impl LlmDriver for ClaudeCodeDriver {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        // Synthreal runs as a service without a login shell.
...
mod tests {
    ...
    fn test_build_prompt_simple() {
        use synthreal_types::message::{Message, MessageContent};
    ...
    fn test_build_prompt_renders_image_attachment_marker() {
        use synthreal_types::message::{ContentBlock, Message, MessageContent};
