...
/// File name for persisted OAuth tokens (inside ~/.synthreal/).
...
impl PersistedTokens {
    ...
    /// Load from the Synthreal data directory.
    pub fn load(synthreal_dir: &Path) -> Option<Self> {
        let path = synthreal_dir.join(TOKEN_FILE_NAME);
        let data = std::fs::read_to_string(&path).ok()?;
        serde_json::from_str(&data).ok()
    }

    /// Persist to the Synthreal data directory with restricted permissions.
    pub fn save(&self, synthreal_dir: &Path) -> Result<(), String> {
        let path = synthreal_dir.join(TOKEN_FILE_NAME);
...
pub async fn exchange_copilot_token(
    ...
    let resp = client
        ...
        .header("Accept", "application/json")
        .header("User-Agent", "Synthreal/1.0")
...
impl CopilotDriver {
    pub fn new(synthreal_dir: PathBuf) -> Self {
        ...
        let persisted = PersistedTokens::load(&synthreal_dir);
    ...
    async fn ensure_access_token(&self) -> Result<String, crate::llm_driver::LlmError> {
        ...
        if let Some(ref rt) = refresh_token {
            match refresh_access_token(&self.http_client, rt).await {
                Ok(new_tokens) => {
                    ...
                    if let Err(e) = new_tokens.save(&self.synthreal_dir) {
        ...
        Err(crate::llm_driver::LlmError::AuthenticationFailed(
            "Copilot OAuth tokens expired. Run `synthreal config set-key github-copilot` to re-authenticate via device flow.".to_string(),
        ))
    }
...
///
/// Called from `synthreal config set-key github-copilot`, `synthreal init`,
/// `synthreal onboard`, and `synthreal configure`.
pub async fn run_interactive_setup(synthreal_dir: &Path) -> Result<PersistedTokens, String> {
    run_device_flow(synthreal_dir).await
}
...
pub async fn run_device_flow(synthreal_dir: &Path) -> Result<PersistedTokens, String> {
...
pub fn copilot_auth_available(synthreal_dir: &Path) -> bool {
    synthreal_dir.join(TOKEN_FILE_NAME).exists()
}
