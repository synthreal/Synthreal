...
use synthreal_types::message::{ContentBlock, Message, StopReason, TokenUsage};
...
pub struct CompletionRequest {
    ...
    pub thinking: Option<synthreal_types::config::ThinkingConfig>,
}
...
pub struct DriverConfig {
    ...
    /// `claude` CLI.  Defaults to `true` because Synthreal runs as a daemon
    /// with no interactive terminal, so permission prompts would block
    /// indefinitely.  Synthreal's own capability / RBAC layer already
    ...
    /// `SYNTHREAL_SUBPROCESS_TIMEOUT_SECS` env var, which wins over both
