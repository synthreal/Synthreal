...
use synthreal_memory::session::Session;
use synthreal_memory::MemorySubstrate;
use synthreal_skills::registry::SkillRegistry;
use synthreal_types::agent::{AgentManifest, FallbackModel};
use synthreal_types::error::{SynthrealError, SynthrealResult};
use synthreal_types::memory::{Memory, MemoryFilter, MemorySource};
use synthreal_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use synthreal_types::tool::{ToolCall, ToolDefinition};
...
/// Overridable via `SYNTHREAL_TOOL_TIMEOUT_SECS` env var. Set to `0` to disable
...
/// Overridable via `SYNTHREAL_AGENT_TOOL_TIMEOUT_SECS` env var. Set to `0` to
...
fn tool_timeout_for(tool_name: &str) -> Option<Duration> {
    let secs = match tool_name {
        "agent_send" | "agent_spawn" => {
            env_timeout_secs("SYNTHREAL_AGENT_TOOL_TIMEOUT_SECS").unwrap_or(AGENT_TOOL_TIMEOUT_SECS)
        }
        _ => env_timeout_secs("SYNTHREAL_TOOL_TIMEOUT_SECS").unwrap_or(TOOL_TIMEOUT_SECS),
    };
    if secs == 0 {
        None
    } else {
...
#[allow(dead_code)]
const MAX_HISTORY_MESSAGES: usize = synthreal_types::agent::DEFAULT_MAX_HISTORY_MESSAGES;
...
pub struct AgentLoopResult {
    ...
    pub directives: synthreal_types::message::ReplyDirectives,
}
...
///
/// This is the core of Synthreal: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
...
) -> SynthrealResult<AgentLoopResult> {
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: synthreal_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if is_silent_token(&text) || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: synthreal_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: synthreal_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in deduplicate_tool_calls(&response) {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(SynthrealError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: synthreal_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                        ...
                        Some(timeout) => {
                            ...
                            match tokio::time::timeout(timeout, exec_fut).await {
                                ...
                                Err(_) => {
                                    ...
                                    synthreal_types::tool::ToolResult {
                                        tool_use_id: tool_call.id.clone(),
                                        content: format!(
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: synthreal_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(SynthrealError::MaxIterationsExceeded(max_iterations))
}
...
) -> SynthrealResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(SynthrealError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(SynthrealError::LlmDriver(user_msg));
            }
        }
    }

    Err(SynthrealError::LlmDriver(
...
mod tests {
    ...
    use synthreal_types::tool::ToolCall;
    ...
    fn test_tool_timeout_for_agent_tools() {
        ...
        std::env::remove_var("SYNTHREAL_AGENT_TOOL_TIMEOUT_SECS");
        std::env::remove_var("SYNTHREAL_TOOL_TIMEOUT_SECS");
        assert_eq!(
        ...
        std::env::set_var("SYNTHREAL_AGENT_TOOL_TIMEOUT_SECS", "0");
        std::env::set_var("SYNTHREAL_TOOL_TIMEOUT_SECS", "0");
        ...
        std::env::set_var("SYNTHREAL_AGENT_TOOL_TIMEOUT_SECS", "1800");
        std::env::set_var("SYNTHREAL_TOOL_TIMEOUT_SECS", "300");
        assert_eq!(
            tool_timeout_for("agent_send"),
        ...
        std::env::set_var("SYNTHREAL_AGENT_TOOL_TIMEOUT_SECS", "not-a-number");
        std::env::set_var("SYNTHREAL_TOOL_TIMEOUT_SECS", "");
        assert_eq!(
            tool_timeout_for("agent_send"),
            Some(Duration::from_secs(600))
        );
    ...
    fn test_max_history_messages() {
        ...
        assert_eq!(
            synthreal_types::agent::DEFAULT_MAX_HISTORY_MESSAGES,
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = synthreal_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = synthreal_types::agent::AgentId::new();
        let mut session = synthreal_memory::session::Session {
            id: synthreal_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
