//! Synthreal CLI — command-line interface for the Synthreal Agent OS.
//!
//! When a daemon is running (`synthreal start`), the CLI talks to it over HTTP.
...
use synthreal_api::server::read_daemon_info;
use synthreal_kernel::SynthrealKernel;
use synthreal_types::agent::{AgentId, AgentManifest};
...
\x1b[1;36mExamples:\x1b[0m
  synthreal init                 Initialize config and data directories
  synthreal start                Start the kernel daemon
  synthreal tui                  Launch the interactive terminal dashboard
  synthreal chat                 Quick chat with the default agent
  synthreal agent new coder      Spawn a new agent from a template
  synthreal models list          Browse available LLM models
  synthreal add github           Install the GitHub integration
  synthreal doctor               Run diagnostic health checks
  synthreal channel setup        Interactive channel setup wizard
  synthreal cron list            List scheduled jobs
  synthreal uninstall            Completely remove Synthreal from your system

\x1b[1;36mQuick Start:\x1b[0m
  1. synthreal init              Set up config + API key
  2. synthreal start             Launch the daemon
  3. synthreal chat              Start chatting!

\x1b[1;36mMore:\x1b[0m
  Docs:       https://github.com/synthreal/Synthreal
  Dashboard:  http://127.0.0.1:4200/ (when daemon is running)";

/// Synthreal — the open-source Agent Operating System.
#[derive(Parser)]
#[command(
    name = "synthreal",
    version,
    about = "\u{1F40D} Synthreal \u{2014} Open-source Agent Operating System",
    long_about = "\u{1F40D} Synthreal \u{2014} Open-source Agent Operating System\n\n\
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize Synthreal (create ~/.synthreal/ and default config).
    Init {
        /// Quick mode: no prompts, just write config + .env (for CI/scripts).
        #[arg(long)]
        quick: bool,
    },
    /// Start the Synthreal kernel daemon (API server + kernel).
    Start {
    ...
    /// Migrate from another agent framework to Synthreal.
    Migrate(MigrateArgs),
    ...
    /// Completely uninstall Synthreal from your system.
    Uninstall {
...
#[derive(Subcommand)]
enum SkillCommands {
    /// Install a skill from SynthHub or a local directory.
    Install {
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.synthreal/.env (prompts interactively).
    SetKey {
...
fn config_log_level() -> String {
    let config_path = if let Ok(home) = std::env::var("SYNTHREAL_HOME") {
...
/// Get the Synthreal home directory, respecting SYNTHREAL_HOME env var.
fn cli_synthreal_home() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("SYNTHREAL_HOME") {
...
fn init_tracing_file() {
    let log_dir = cli_synthreal_home();
...
fn main() {
    // Load ~/.synthreal/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
...
pub(crate) fn find_daemon() -> Option<String> {
    let home_dir = cli_synthreal_home();
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                ui::error_with_fix(
                    ...
                    "Check daemon logs: ~/.synthreal/tui.log",
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `synthreal status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: synthreal start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `synthreal status` or restart: synthreal start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool) {
    ...
    let synthreal_dir = cli_synthreal_home();

    // --- Ensure directories exist ---
    if !synthreal_dir.exists() {
        std::fs::create_dir_all(&synthreal_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", synthreal_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&synthreal_dir);
    }

    for sub in ["data", "agents"] {
        let dir = synthreal_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&synthreal_dir.join("agents"));

    if quick {
        cmd_init_quick(&synthreal_dir);
    } else if !std::io::IsTerminal::is_terminal(&std::io::stdin())
        || !std::io::IsTerminal::is_terminal(&std::io::stdout())
    {
        ui::hint("Non-interactive terminal detected — running in quick mode");
        ui::hint("For the interactive wizard, run: synthreal init (in a terminal)");
        cmd_init_quick(&synthreal_dir);
    } else {
        cmd_init_interactive(&synthreal_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(synthreal_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(synthreal_dir, provider, model, api_key_env);

    ui::blank();
    ui::success("Synthreal initialized (quick mode)");
    ui::kv("Provider", provider);
    ui::kv("Model", model);
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  synthreal start",
        "Chat:              synthreal chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(synthreal_dir: &std::path::Path) {
    ...
    match init_wizard::run() {
        InitResult::Completed {
            ...
            ui::blank();
            ui::success("Synthreal initialized!");
            ui::kv("Provider", &provider);
            ui::kv("Model", &model);
            ...
            match launch {
                LaunchChoice::Desktop => {
                    launch_desktop_app(synthreal_dir);
                }
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: synthreal start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the synthreal-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_synthreal_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "synthreal-desktop.exe";
        #[cfg(not(windows))]
        let name = "synthreal-desktop";
    ...
    match desktop_bin {
        Some(ref path) if path.exists() => {
            ui::success("Launching Synthreal Desktop...");
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: synthreal dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install synthreal-desktop");
            ...
            if let Some(base) = find_daemon() {
                ...
                ui::hint("Daemon is not running. Start it with: synthreal start");
...
fn write_config_if_missing(
    synthreal_dir: &std::path::Path,
    provider: &str,
    model: &str,
    api_key_env: &str,
) {
    let config_path = synthreal_dir.join("config.toml");
    if config_path.exists() {
        ui::check_ok(&format!("Config already exists: {}", config_path.display()));
    } else {
        let default_config = format!(
            r#"# Synthreal Agent OS configuration
# See https://github.com/synthreal/Synthreal for documentation

# For Docker, change to "0.0.0.0:4200" or set SYNTHREAL_LISTEN env var.
...
fn cmd_start(config: Option<PathBuf>, yolo: bool) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `synthreal status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        let mut kernel_config = synthreal_kernel::config::load_config(config.as_deref());
        if yolo {
            kernel_config.approval.auto_approve = true;
            kernel_config.approval.apply_shorthands();
        }
        let kernel = match SynthrealKernel::boot_with_config(kernel_config) {
            Ok(k) => k,
            Err(e) => {
                boot_kernel_error(&e);
                std::process::exit(1);
        ...
        ui::kv("Model", &model);
        ui::blank();
        ui::hint("Open the dashboard in your browser, or run `synthreal chat`");
        ui::hint("Press Ctrl+C to stop the daemon");
        ui::blank();

        if let Err(e) =
            synthreal_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
        ...
        ui::blank();
        println!("  Synthreal daemon stopped.");
    });
}

/// Read the api_key from ~/.synthreal/config.toml (if any).
///
...
fn read_api_key() -> Option<String> {
    ...
    let config_path = cli_synthreal_home().join("config.toml");
    ...
    // 2. Fall back to SYNTHREAL_API_KEY env var
    ...
    match find_daemon() {
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: synthreal status",
            );
        }
    }
}
...
fn boot_kernel_error(e: &synthreal_kernel::error::KernelError) {
    let msg = e.to_string();
    if msg.contains("parse") || msg.contains("toml") || msg.contains("config") {
        ui::error_with_fix(
            "Failed to parse configuration",
            "Check your config.toml syntax: synthreal config show",
        );
    } else if msg.contains("database") || msg.contains("locked") || msg.contains("sqlite") {
        ui::error_with_fix(
            "Database error (file may be locked)",
            "Check if another Synthreal process is running: synthreal status",
        );
    } else if msg.contains("key") || msg.contains("API") || msg.contains("auth") {
        ui::error_with_fix(
            "LLM provider authentication failed",
            "Run `synthreal doctor` to check your API key configuration",
        );
    } else {
        ui::error_with_fix(
            &format!("Failed to boot kernel: {msg}"),
            "Run `synthreal doctor` to diagnose the issue",
        );
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use `synthreal agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use `synthreal start` first.");
            }
            Err(e) => {
    ...
    match field {
        "model" => {
            if let Some(base) = find_daemon() {
                ...
                eprintln!("No running daemon found. Start one with: synthreal start");
                std::process::exit(1);
            }
        }
        _ => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run `synthreal init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run `synthreal agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: synthreal chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use `synthreal start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("Synthreal Daemon Status");
        ui::blank();
        ...
        ui::section("Synthreal Status (in-process)");
        ui::blank();
        ...
        ui::blank();
        ui::hint("Run `synthreal start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(_h) = &home {
        ...
        // --- Check 1: Synthreal directory ---
        if synthreal_dir.exists() {
            if !json {
                ui::check_ok(&format!("Synthreal directory: {}", synthreal_dir.display()));
            }
            checks.push(serde_json::json!({"check": "synthreal_dir", "status": "ok", "path": synthreal_dir.display().to_string()}));
        } else if repair {
            if !json {
                ui::check_fail("Synthreal directory not found.");
            }
            let answer = prompt_input("    Create it now? [Y/n] ");
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&synthreal_dir).is_ok() {
                    restrict_dir_permissions(&synthreal_dir);
                    for sub in ["data", "agents"] {
                        let _ = std::fs::create_dir_all(synthreal_dir.join(sub));
                    }
                    if !json {
                        ui::check_ok("Created Synthreal directory");
                    }
                    repaired = true;
                } else {
                    if !json {
            ...
            checks.push(serde_json::json!({"check": "synthreal_dir", "status": if repaired { "repaired" } else { "fail" }}));
        } else {
            if !json {
                ui::check_fail("Synthreal directory not found. Run `synthreal init` first.");
            }
            checks.push(serde_json::json!({"check": "synthreal_dir", "status": "fail"}));
            all_ok = false;
        }

        // --- Check 2: .env file exists + permissions ---
        let env_path = synthreal_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: synthreal config set-key <provider>)",
                );
            }
            ...
            match toml::from_str::<toml::Value>(&config_content) {
                ...
                Err(e) => {
                    if !json {
                        ...
                        ui::hint("Fix with: synthreal config edit");
                    }
...
"#
                ...
                let _ = std::fs::create_dir_all(&synthreal_dir);
        ...
        let api_listen = {
            let cfg_path = synthreal_dir.join("config.toml");
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<synthreal_types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
        ...
        if let Some(ref base) = daemon_running {
            ...
            if !json {
                ui::check_warn("Daemon not running (start with `synthreal start`)");
            }
        ...
        let daemon_json_path = synthreal_dir.join("daemon.json");
        ...
        let db_path = synthreal_dir.join("data").join("synthreal.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &synthreal_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = synthreal_dir.join("agents");
    ...
    {
        let synthreal_dir = cli_synthreal_home();
        if synthreal_runtime::drivers::copilot::copilot_auth_available(&synthreal_dir) {
            any_key_set = true;
            if !json {
    ...
    if !any_key_set {
        if !json {
            ...
            ui::blank();
            ui::hint("Or run: synthreal config set-key groq");
        }
        all_ok = false;
    }
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<synthreal_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = synthreal_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                synthreal_types::config::McpTransportEntry::Stdio {
                                    command,
                                    ..
                                } => {
                                    if command.is_empty() {
                                ...
                                synthreal_types::config::McpTransportEntry::Sse { url }
                                | synthreal_types::config::McpTransportEntry::Http { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = cli_synthreal_home().join("skills");
        let mut skill_reg = synthreal_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings = synthreal_skills::verify::SkillVerifier::scan_prompt_content(prompt);
                let has_critical = warnings.iter().any(|w| {
                    matches!(
                        w.severity,
                        synthreal_skills::verify::WarningSeverity::Critical
                    )
                });
                if has_critical {
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let mut ext_registry =
            synthreal_extensions::registry::IntegrationRegistry::new(&synthreal_dir);
    ...
    if json {
        ...
        println!();
        if all_ok {
            ui::success("All checks passed! Synthreal is ready.");
            if find_daemon().is_none() {
                ui::hint("Start the daemon: synthreal start");
            }
        } else if repaired {
            ui::success("Repairs applied. Re-run `synthreal doctor` to verify.");
        } else {
            ui::error("Some checks failed.");
            if !repair {
                ui::hint("Run `synthreal doctor --repair` to attempt auto-fix");
            }
        }
    }
}
        ...
        match start_daemon_background() {
            ...
            Err(e) => {
                ui::error_with_fix(
                    ...
                    "Start it manually: synthreal start",
                );
                std::process::exit(1);
            }
        }
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "synthreal", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`synthreal {command}` requires a running daemon"),
            "Start the daemon: synthreal start",
        );
        ui::hint("Or try `synthreal chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> SynthrealKernel {
    match SynthrealKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => synthreal_migrate::MigrateSource::OpenClaw,
        MigrateSourceArg::Langchain => synthreal_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => synthreal_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            synthreal_migrate::MigrateSource::OpenClaw => home.join(".openclaw"),
            synthreal_migrate::MigrateSource::LangChain => home.join(".langchain"),
            synthreal_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });

    let target_dir = cli_synthreal_home();
    ...
    let options = synthreal_migrate::MigrateOptions {
        source,
        source_dir,
        target_dir,
    ...
    match synthreal_migrate::run_migration(&options) {
        Ok(report) => {
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if synthreal_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match synthreal_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
                        // Copy skill directory
                        copy_dir_recursive(&source_path, &dest);
                        if let Err(e) = synthreal_skills::openclaw_compat::write_synthreal_manifest(
                            &dest, &manifest,
                        ) {
        ...
        let manifest: synthreal_skills::SkillManifest =
        ...
        || source.starts_with("git@")
    {
        ...
        if !manifest_path.exists() {
            if synthreal_skills::openclaw_compat::detect_openclaw_skill(&clone_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match synthreal_skills::openclaw_compat::convert_openclaw_skill(&clone_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
        ...
        // Remote install from SynthHub
        println!("Installing {source} from SynthHub...");
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = synthreal_skills::marketplace::MarketplaceClient::new(
            synthreal_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn notify_daemon_skill_reload() {
    if let Some(base) = find_daemon() {
        ...
        match client.post(format!("{base}/api/skills/reload")).send() {
            ...
            _ => {
                ui::check_warn("Could not notify daemon. Restart with: synthreal restart");
            }
        }
    } else {
        ui::hint("Start the daemon to make this skill available to agents: synthreal start");
    }
}
...
fn cmd_skill_list() {
    ...
    let mut registry = synthreal_skills::registry::SkillRegistry::new(skills_dir);
...
"#
    ...
    println!("  2. Test locally: synthreal skill test");
    println!(
        "  3. Install: synthreal skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run `synthreal init` first.");
        return;
    }
    ...
    println!("\nUse `synthreal channel setup <channel>` to configure a channel.");
}
    ...
    match channel.as_str() {
        "telegram" => {
            ...
            match dotenv::save_env_key("TELEGRAM_BOT_TOKEN", &token) {
                Ok(()) => ui::success("Token saved to ~/.synthreal/.env"),
            ...
            for (key, val) in [
                ...
                if !val.is_empty() {
                    match dotenv::save_env_key(key, val) {
                        Ok(()) => ui::success(&format!("{key} saved to ~/.synthreal/.env")),
        ...
        "email" => {
            ...
            if !password.is_empty() {
                match dotenv::save_env_key("EMAIL_PASSWORD", &password) {
                    Ok(()) => ui::success("Password saved to ~/.synthreal/.env"),
                    Err(_) => println!("    export EMAIL_PASSWORD=your_app_password"),
                }
            } else {
                ui::hint("Set later: synthreal config set-key email (or export EMAIL_PASSWORD=...)");
            }
        ...
        "signal" => {
            ...
            if !phone.is_empty() {
                match dotenv::save_env_key("SIGNAL_PHONE", &phone) {
                    Ok(()) => ui::success("Phone saved to ~/.synthreal/.env"),
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @synthreal-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"synthreal-bot\",\"password\":\"...\"}}'");
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run `synthreal init` first.");
        return;
    }
...
fn cmd_channel_test(channel: &str) {
    if let Some(base) = find_daemon() {
        ...
        eprintln!("Channel test requires a running daemon. Start with: synthreal start");
        std::process::exit(1);
    }
}
...
fn cmd_channel_toggle(channel: &str, enable: bool) {
    ...
    if let Some(base) = find_daemon() {
        ...
        println!("Edit ~/.synthreal/config.toml to persist this change.");
    }
}
...
fn cmd_hand_install(path: &str) {
    ...
    println!(
        "Use `synthreal hand activate {}` to start it.",
        body["id"].as_str().unwrap_or("?")
    );
}
...
fn cmd_hand_list() {
    ...
    if let Some(arr) = Some(&arr_val) {
        ...
        println!("\nUse `synthreal hand activate <id>` to activate a hand.");
    }
}
...
fn cmd_hand_config(
    ...
    if let Some(err) = resp.get("error").and_then(|v| v.as_str()) {
        ...
        if err.contains("No active instance") {
            ui::hint(&format!(
                "Activate the hand first: synthreal hand activate {id}"
            ));
        }
        std::process::exit(1);
    }
...
/// Spawn `synthreal start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run `synthreal init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run `synthreal init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run `synthreal config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    if provider == "github-copilot" || provider == "copilot" {
        ...
        match rt.block_on(synthreal_runtime::drivers::copilot::run_interactive_setup(
            &synthreal_dir,
        )) {
            Ok(_) => {
                ui::success("GitHub Copilot configured successfully");
                ui::hint("Restart the daemon: synthreal stop && synthreal start");
            }
            Err(e) => {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.synthreal/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    // Remove from vault (best-effort)
    {
        ...
        if vault_path.exists() {
            let mut vault = synthreal_extensions::vault::CredentialVault::new(vault_path);
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.synthreal/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if std::env::var(&env_var).is_err() {
        ...
        ui::hint(&format!("Set it: synthreal config set-key {provider}"));
        std::process::exit(1);
    }
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: synthreal config set-key {provider}"));
        std::process::exit(1);
    }
}
...
pub(crate) fn synthreal_home() -> PathBuf {
    if let Ok(home) = std::env::var("SYNTHREAL_HOME") {
...
// ---------------------------------------------------------------------------
// Integration commands (synthreal add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = synthreal_home();
    let mut registry = synthreal_extensions::registry::IntegrationRegistry::new(&home);
    ...
    match synthreal_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                synthreal_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                synthreal_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  synthreal vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match synthreal_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        synthreal_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        synthreal_extensions::installer::search_integrations(&registry, q)
    } else {
        synthreal_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&synthreal_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                synthreal_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                synthreal_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                synthreal_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                synthreal_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                synthreal_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `synthreal add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (synthreal vault init/set/list/remove)
// ---------------------------------------------------------------------------
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: synthreal vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: synthreal vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (synthreal new skill/integration)
// ---------------------------------------------------------------------------
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            synthreal_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            synthreal_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = synthreal_runtime::model_catalog::ModelCatalog::new();
...
fn pick_model() -> String {
    ...
    let mut by_provider: std::collections::BTreeMap<
        String,
        Vec<&synthreal_types::model_catalog::ModelCatalogEntry>,
    > = std::collections::BTreeMap::new();
    for m in models {
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = cli_synthreal_home().join("tui.log");
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: synthreal start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_auth_hash_password() {
    ...
    let hash = synthreal_api::session_auth::hash_password(&password);
    println!();
...
fn cmd_devices_pair() {
    ...
    if let Some(qr) = body["qr_data"].as_str() {
        ...
        println!("  Scan this QR code with the Synthreal mobile app:");
        ui::blank();
        println!("  {qr}");
        ui::blank();
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("Synthreal System Info");
        ui::blank();
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: synthreal start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("synthreal {}", env!("CARGO_PKG_VERSION"));
}
...
fn cmd_reset(confirm: bool) {
    ...
    if !synthreal_dir.exists() {
        println!(
            ...
            synthreal_dir.display()
        );
        return;
    }

    if !confirm {
        println!("  This will delete all data in {}", synthreal_dir.display());
    ...
    match std::fs::remove_dir_all(&synthreal_dir) {
        Ok(()) => ui::success(&format!("Removed {}", synthreal_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", synthreal_dir.display()));
            std::process::exit(1);
        }
    }
}
...
fn cmd_uninstall(confirm: bool, keep_config: bool) {
    ...
    println!(
        "  {}",
        "This will completely uninstall Synthreal from your system."
            .bold()
            .red()
    );
    println!();
    if synthreal_dir.exists() {
        if keep_config {
            ...
            println!("  • Remove {}", synthreal_dir.display());
        }
    }
    ...
    if find_daemon().is_some() {
        ...
        if find_daemon().is_some() {
            if let Some(info) = read_daemon_info(&synthreal_dir) {
                force_kill_pid(info.pid);
                let _ = std::fs::remove_file(synthreal_dir.join("daemon.json"));
            }
        }
    }
    ...
    // Step 6: Remove ~/.synthreal/ data
    if synthreal_dir.exists() {
        if keep_config {
            remove_dir_except_config(&synthreal_dir);
            ui::success("Removed data (kept config files)");
        } else {
            match std::fs::remove_dir_all(&synthreal_dir) {
                Ok(()) => ui::success(&format!("Removed {}", synthreal_dir.display())),
                Err(e) => ui::error(&format!("Failed to remove {}: {e}", synthreal_dir.display())),
            }
        }
    }
    ...
    println!();
    ui::success("Synthreal has been uninstalled. Goodbye!");
}
...
fn remove_autostart_entries(home: &std::path::Path) {
    ...
    #[cfg(target_os = "macos")]
    {
        let plist = home.join("Library/LaunchAgents/ai.synthreal.desktop.plist");
        if plist.exists() {
            // Unload first
    ...
    #[cfg(target_os = "linux")]
    {
        let desktop_file = home.join(".config/autostart/Synthreal.desktop");
        ...
        let service_file = home.join(".config/systemd/user/synthreal.service");
        if service_file.exists() {
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "synthreal.service"])
                .output();
...
/// Remove lines from shell config files that add synthreal to PATH.
#[allow(unused_variables)]
fn clean_path_entries(home: &std::path::Path, synthreal_dir: &str) {
    #[cfg(not(windows))]
    {
        let shell_files = [
            home.join(".bashrc"),
        ...
        for path in &shell_files {
            ...
            let filtered: Vec<&str> = content
                .lines()
                .filter(|line| !is_synthreal_path_line(line, synthreal_dir))
                .collect();
    ...
    #[cfg(windows)]
    {
        // Read User PATH via PowerShell, filter out synthreal entries, write back
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                if !current.is_empty() {
                    let dir_lower = synthreal_dir.to_lowercase();
                    let filtered: Vec<&str> = current
                        .split(';')
                        .filter(|entry| {
                            let e = entry.trim().to_lowercase();
                            !e.is_empty() && !e.contains("synthreal") && !e.contains(&dir_lower)
                        })
                        .collect();
...
/// Returns true if a shell config line is an synthreal PATH export.
/// Must match BOTH an synthreal reference AND a PATH-setting pattern.
#[cfg(any(not(windows), test))]
fn is_synthreal_path_line(line: &str, synthreal_dir: &str) -> bool {
    let lower = line.to_lowercase();
    let has_synthreal = lower.contains("synthreal") || lower.contains(&synthreal_dir.to_lowercase());
    if !has_synthreal {
        return false;
    }
...
/// Remove everything in ~/.synthreal/ except config files.
fn remove_dir_except_config(synthreal_dir: &std::path::Path) {
    let keep = ["config.toml", ".env", "secrets.env"];
    let Ok(entries) = std::fs::read_dir(synthreal_dir) else {
        return;
    };
    for entry in entries.flatten() {
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("synthreal-doctor-test-skills");
        let mut skill_reg = synthreal_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("synthreal-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = synthreal_extensions::registry::IntegrationRegistry::new(&tmp);
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = synthreal_types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: synthreal_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: synthreal_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            synthreal_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            synthreal_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = synthreal_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
    ...
    fn test_doctor_hook_event_variants() {
        ...
        use synthreal_types::agent::HookEvent;
        let events = [
...
"#;
    ...
    fn test_uninstall_path_line_filter() {
        use super::is_synthreal_path_line;
        let dir = "/home/user/.synthreal/bin";

        // Should match: synthreal PATH exports
        assert!(is_synthreal_path_line(
            r#"export PATH="$HOME/.synthreal/bin:$PATH""#,
            dir
        ));
        assert!(is_synthreal_path_line(
            r#"export PATH="/home/user/.synthreal/bin:$PATH""#,
            dir
        ));
        assert!(is_synthreal_path_line(
            "set -gx PATH $HOME/.synthreal/bin $PATH",
            dir
        ));
        assert!(is_synthreal_path_line(
            "fish_add_path $HOME/.synthreal/bin",
            dir
        ));
        ...
        // Should NOT match: synthreal lines that aren't PATH-related
        assert!(!is_synthreal_path_line("# synthreal config", dir));
        assert!(!is_synthreal_path_line("alias of=synthreal", dir));
    }
}
