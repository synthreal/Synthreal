//! Standalone chat TUI for `synthreal chat`.
//!
...
enum Backend {
    ...
    InProcess { kernel: Arc<SynthrealKernel> },
    None,
}
...
impl StandaloneChat {
    ...
    fn handle_stream_done(
        &mut self,
        result: Result<synthreal_runtime::agent_loop::AgentLoopResult, String>,
    ) {
    ...
    fn handle_kernel_ready(&mut self, kernel: Arc<SynthrealKernel>) {
        self.booting = false;
    ...
    fn resolve_daemon_agent(&mut self, base_url: &str, agent_name: Option<&str>) {
        ...
        match template {
            ...
            None => {
                self.boot_error =
                    Some("No agent templates found. Run `synthreal init`.".to_string());
            }
        }
    }
