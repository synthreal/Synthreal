...
//!
//! Launched by `synthreal init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use synthreal_runtime::model_catalog::ModelCatalog;
use synthreal_types::model_catalog::ModelTier;
...
mod tests {
    ...
    fn init_wizard_lists_minimax_provider() {
        ...
        assert!(
            minimax.is_some(),
            "MiniMax should be selectable in synthreal init"
        );
...
struct State {
    ...
    openclaw_scan: Option<synthreal_migrate::openclaw::ScanResult>,
    migration_report: Option<synthreal_migrate::report::MigrationReport>,
...
impl State {
    ...
    fn build_provider_order(&mut self) {
        ...
        for (i, p) in PROVIDERS.iter().enumerate() {
            let detected = if p.name == "claude-code" {
                synthreal_runtime::drivers::claude_code::claude_code_available()
            } else {
    ...
    fn is_provider_detected(&self, prov_idx: usize) -> bool {
        ...
        if p.name == "claude-code" {
            return synthreal_runtime::drivers::claude_code::claude_code_available();
        }
        (!p.env_var.is_empty() && std::env::var(p.env_var).is_ok())
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<synthreal_migrate::report::MigrationReport, String>>();
            ...
            if let Ok(CtEvent::Key(key)) = event::read() {
                ...
                match state.step {
                            ...
                            if let Some(list_idx) = state.provider_list.selected() {
                                ...
                                if p.name == "github-copilot" {
                                    ...
                                    std::thread::spawn(move || {
                                        let synthreal_dir = crate::cli_synthreal_home();
                                        ...
                                        rt.block_on(async {
                                            ...
                                            // Save tokens
                                            if let Err(e) = tokens.save(&synthreal_dir) {
                                                let _ = copilot_tx.send(Err(e));
                                                return;
                                            }
                    ...
                    Step::CopilotAuth => match key.code {
                        ...
                        {
                            let _ = synthreal_runtime::drivers::copilot::open_verification_url(
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<synthreal_migrate::report::MigrationReport, String>>,
) {
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let target_dir = if let Ok(h) = std::env::var("SYNTHREAL_HOME") {
                        PathBuf::from(h)
                    } else {
                        dirs::home_dir()
                    ...
                    std::thread::spawn(move || {
                        let options = synthreal_migrate::MigrateOptions {
                            source: synthreal_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            synthreal_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let synthreal_dir = if let Ok(h) = std::env::var("SYNTHREAL_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".synthreal"),
            None => {
    ...
    let _ = std::fs::create_dir_all(synthreal_dir.join("agents"));
    let _ = std::fs::create_dir_all(synthreal_dir.join("data"));
    crate::restrict_dir_permissions(&synthreal_dir);
...
/// Check if the `synthreal-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "Synthreal Init  Step X of 7"
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use synthreal_migrate::report::ItemKind;
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = synthreal_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use Synthreal?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
