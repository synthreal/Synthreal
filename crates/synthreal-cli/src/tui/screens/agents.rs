...
pub struct InProcessAgent {
    pub id: synthreal_types::agent::AgentId,
    pub name: String,
    pub state: String,
    pub provider: String,
    pub model: String,
...
impl AgentSelectState {
    ...
    pub fn load_inprocess_agents(&mut self, kernel: &synthreal_kernel::SynthrealKernel) {
...
fn truncate(s: &str, max: usize) -> String {
    if s.len() <= max {
        ...
        format!(
            "{}\u{2026}",
            synthreal_types::truncate_str(s, max.saturating_sub(1))
        )
    }
}
