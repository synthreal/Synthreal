...
/// 2. `~/.synthreal/agents/` (installed templates)
/// 3. `SYNTHREAL_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects SYNTHREAL_HOME)
    let of_home = if let Ok(h) = std::env::var("SYNTHREAL_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".synthreal")
    } else {
        std::env::temp_dir().join(".synthreal")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("SYNTHREAL_AGENTS_DIR") {
