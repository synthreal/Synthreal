//! Minimal `.env` file loader/saver for `~/.synthreal/.env`.
//!
...
fn dotenv_synthreal_home() -> Option<PathBuf> {
    if let Ok(home) = std::env::var("SYNTHREAL_HOME") {
        return Some(PathBuf::from(home));
    }
    dirs::home_dir().map(|h| h.join(".synthreal"))
}

/// Return the path to `~/.synthreal/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dotenv_synthreal_home().map(|h| h.join(".env"))
}

/// Load `~/.synthreal/.env` and `~/.synthreal/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.synthreal/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dotenv_synthreal_home().map(|h| h.join("secrets.env"))
}
...
/// Remove a key from `~/.synthreal/.env`.
///
...
/// List key names (without values) from `~/.synthreal/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# Synthreal environment — managed by `synthreal config set-key`\n");
