//! Real HTTP integration tests for the Synthreal API.
//!
...
//!
//! Run: cargo test -p synthreal-api --test api_integration_test -- --nocapture

use axum::Router;
use synthreal_api::middleware;
use synthreal_api::routes::{self, AppState};
use synthreal_api::ws;
use synthreal_kernel::SynthrealKernel;
use synthreal_types::config::{DefaultModelConfig, KernelConfig};
use std::sync::Arc;
...
async fn test_list_agents_includes_inferencing_flag() {
    ...
    let agent_id: synthreal_types::agent::AgentId = agent_id_str.parse().unwrap();
...
async fn test_agent_session_filters_system_messages() {
    use synthreal_types::message::{Message, Role};
    ...
    session.messages = vec![
        Message {
            role: Role::System,
            content: synthreal_types::message::MessageContent::Text(
...
async fn test_schedules_delivery_targets_roundtrip() {
    ...
    let delivery_targets = serde_json::json!([
        ...
        { "type": "local_file", "path": "/tmp/synthreal-test.log", "append": true },
