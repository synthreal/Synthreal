...
//! the wire contract.
//!
//! Run: cargo test -p synthreal-api --test skill_config_api_test -- --nocapture
...
config:
  github_token:
    ...
    env: SYNTHREAL_TEST_SKILLCFG_GH_TOKEN
    required: true
  default_branch:
...
async fn start_test_server() -> TestServer {
    ...
    let kernel = SynthrealKernel::boot_with_config(config).expect("kernel boot");
...
async fn get_config_returns_declared_and_resolved() {
    ...
    unsafe { std::env::remove_var("SYNTHREAL_TEST_SKILLCFG_GH_TOKEN") };
