//! Route handlers for the Synthreal API.
...
use axum::Json;
use dashmap::DashMap;
use synthreal_kernel::triggers::{TriggerId, TriggerPattern};
use synthreal_kernel::workflow::{
...
use synthreal_types::agent::{AgentId, AgentIdentity, AgentManifest};
...
pub struct AppState {
    pub kernel: Arc<SynthrealKernel>,
    pub started_at: Instant,
    /// Optional peer registry for OFP mesh networking status.
    pub peer_registry: Option<Arc<synthreal_wire::registry::PeerRegistry>>,
    /// Channel bridge manager — held behind a Mutex so it can be swapped on hot-reload.
    pub bridge_manager: tokio::sync::Mutex<Option<synthreal_channels::bridge::BridgeManager>>,
    /// Live channel config — updated on every hot-reload so list_channels() reflects reality.
    pub channels_config: tokio::sync::RwLock<synthreal_types::config::ChannelsConfig>,
    ...
    pub provider_probe_cache: synthreal_runtime::provider_health::ProbeCache,
    /// Thread-safe mutable budget config. Updated via PUT /api/budget.
    /// Initialized from `kernel.config.budget` at startup.
    pub budget_config: Arc<tokio::sync::RwLock<synthreal_types::config::BudgetConfig>>,
}
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    synthreal_runtime::audit::AuditAction::AuthAttempt,
...
pub async fn list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let agents: Vec<serde_json::Value> = state
        ...
        .map(|e| {
            ...
            let ready = matches!(e.state, synthreal_types::agent::AgentState::Running)
                && auth_status != "missing";
...
) -> Vec<synthreal_types::message::ContentBlock> {
    use base64::Engine;

    let upload_dir = std::env::temp_dir().join("synthreal_uploads");
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(synthreal_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    ...
    agent_id: AgentId,
    image_blocks: Vec<synthreal_types::message::ContentBlock>,
) {
    use synthreal_types::message::{Message, MessageContent, Role};
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => synthreal_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
...
pub async fn get_agent_session(
    ...
    match state.kernel.memory.get_session(entry.session_id) {
        Ok(Some(session)) => {
            ...
            let filtered_messages: Vec<&synthreal_types::message::Message> = session
                .messages
                .iter()
                .filter(|m| include_system || m.role != synthreal_types::message::Role::System)
                .collect();
            ...
            for m in &filtered_messages {
                ...
                let content = match &m.content {
                    synthreal_types::message::MessageContent::Text(t) => t.clone(),
                    synthreal_types::message::MessageContent::Blocks(blocks) => {
                        let mut texts = Vec::new();
                        for b in blocks {
                            match b {
                                synthreal_types::message::ContentBlock::Text { text, .. } => {
                                    texts.push(text.clone());
                                }
                                synthreal_types::message::ContentBlock::Image {
                                    media_type,
                                    data,
                                } => {
                                ...
                                synthreal_types::message::ContentBlock::ToolUse {
                                    id,
                                    name,
                                    input,
                                    ..
                                ...
                                synthreal_types::message::ContentBlock::ToolResult { .. } => {}
                                _ => {}
                            }
                        }
                        texts.join("\n")
            ...
            for m in &filtered_messages {
                if let synthreal_types::message::MessageContent::Blocks(blocks) = &m.content {
                    for b in blocks {
                        if let synthreal_types::message::ContentBlock::ToolResult {
                            tool_use_id,
                            content: result,
                            is_error,
                            ..
...
/// this also removes the on-disk `~/.synthreal/agents/<name>/` directory so
...
pub async fn uninstall_agent(
    ...
    // Step 2: remove ~/.synthreal/agents/<name>/ so the agent does NOT
...
pub async fn restart_agent(
    ...
    let _ = state
        .kernel
        .registry
        .set_state(agent_id, synthreal_types::agent::AgentState::Running);
...
pub async fn send_message_stream(
    ...
    use futures::stream;
    use synthreal_runtime::llm_driver::StreamEvent;
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@synthreal:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "synthreal_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "synthreal_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "synthreal, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "synthreal", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#synthreal, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "synthreal", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "synthreal-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "synthreal", advanced: false },
...
fn is_channel_configured(config: &synthreal_types::config::ChannelsConfig, name: &str) -> bool {
    match name {
...
fn channel_config_values(
    config: &synthreal_types::config::ChannelsConfig,
    name: &str,
...
pub async fn configure_channel(
    ...
    let home = synthreal_kernel::config::synthreal_home();
...
async fn send_channel_test_message(channel_name: &str, target_id: &str) -> Result<(), String> {
    ...
    let test_msg = "Synthreal test message — your channel is connected!";
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = synthreal_kernel::config::synthreal_home().join("agents");
...
pub async fn get_agent_kv(
    ...
    let agent_id = synthreal_kernel::kernel::shared_memory_agent_id();
...
pub async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let db_ok = tokio::task::spawn_blocking(move || {
        let shared_id = synthreal_types::agent::AgentId(uuid::Uuid::from_bytes([
...
///
/// Returns counters and gauges for monitoring Synthreal in production:
/// - `synthreal_agents_active` — number of active agents
/// - `synthreal_uptime_seconds` — seconds since daemon started
/// - `synthreal_tokens_total` — total tokens consumed (per agent)
/// - `synthreal_tool_calls_total` — total tool calls (per agent)
/// - `synthreal_panics_total` — supervisor panic count
/// - `synthreal_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP synthreal_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE synthreal_uptime_seconds gauge\n");
    out.push_str(&format!("synthreal_uptime_seconds {uptime}\n\n"));

    // Active agents
    let agents = state.kernel.registry.list();
    let active = agents
        .iter()
        .filter(|a| matches!(a.state, synthreal_types::agent::AgentState::Running))
        .count();
    out.push_str("# HELP synthreal_agents_active Number of active agents.\n");
    out.push_str("# TYPE synthreal_agents_active gauge\n");
    out.push_str(&format!("synthreal_agents_active {active}\n"));
    out.push_str("# HELP synthreal_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE synthreal_agents_total gauge\n");
    out.push_str(&format!("synthreal_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP synthreal_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE synthreal_tokens_total gauge\n");
    out.push_str("# HELP synthreal_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE synthreal_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "synthreal_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "synthreal_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP synthreal_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE synthreal_panics_total counter\n");
    out.push_str(&format!("synthreal_panics_total {}\n", health.panic_count));
    out.push_str("# HELP synthreal_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE synthreal_restarts_total counter\n");
    out.push_str(&format!(
        "synthreal_restarts_total {}\n\n",
        health.restart_count
    ));

    // Version info
    out.push_str("# HELP synthreal_info Synthreal version and build info.\n");
    out.push_str("# TYPE synthreal_info gauge\n");
    out.push_str(&format!(
        "synthreal_info{{version=\"{}\"}} 1\n",
...
/// POST /api/skills/install — Install a skill from SynthHub (GitHub).
pub async fn install_skill(
    State(state): State<Arc<AppState>>,
    Json(req): Json<SkillInstallRequest>,
) -> impl IntoResponse {
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = synthreal_skills::marketplace::MarketplaceConfig::default();
    let client = synthreal_skills::marketplace::MarketplaceClient::new(config);

    let opts = synthreal_skills::installer::InstallOptions {
...
///
/// Called by the CLI after `synthreal skill install` to notify the running
...
pub async fn audit_append(
    ...
    use synthreal_runtime::audit::AuditAction;
...
/// GET /api/marketplace/search — Search the SynthHub marketplace.
...
pub async fn clawhub_search(
    ...
    let client = synthreal_skills::clawhub::ClawHubClient::new(cache_dir);
...
pub async fn clawhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => synthreal_skills::clawhub::ClawHubSort::Downloads,
        Some("stars") => synthreal_skills::clawhub::ClawHubSort::Stars,
        Some("updated") => synthreal_skills::clawhub::ClawHubSort::Updated,
        Some("rating") => synthreal_skills::clawhub::ClawHubSort::Rating,
        _ => synthreal_skills::clawhub::ClawHubSort::Trending,
    };
...
pub async fn clawhub_install(
    ...
    match client.install(&req.slug, &skills_dir).await {
        ...
        Err(e) => {
            let msg = format!("{e}");
            let status = if matches!(e, synthreal_skills::SkillError::SecurityBlocked(_)) {
                StatusCode::FORBIDDEN
            } else if is_clawhub_rate_limit(&e) {
                StatusCode::TOO_MANY_REQUESTS
            } else if matches!(e, synthreal_skills::SkillError::Network(_)) {
...
fn is_clawhub_rate_limit(err: &synthreal_skills::SkillError) -> bool {
    matches!(err, synthreal_skills::SkillError::RateLimited(_))
}

/// Convert a browse entry (nested stats/tags) to a flat JSON object for the frontend.
fn clawhub_browse_entry_to_json(
    entry: &synthreal_skills::clawhub::ClawHubBrowseEntry,
) -> serde_json::Value {
    let version = synthreal_skills::clawhub::ClawHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<synthreal_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
            ...
            if let Some(agent_id) = instance.agent_id {
                ...
                if let Some(entry) = entry {
                    if !matches!(
                        ...
                        synthreal_types::agent::ScheduleMode::Reactive
                    ) {
...
pub async fn hand_stats(
    ...
    let shared_id = synthreal_kernel::kernel::shared_memory_agent_id();
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            synthreal_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!(
                        "{}... (truncated)",
                        synthreal_types::truncate_str(&content, 2000)
                    );
                }
            }
        }
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            synthreal_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
pub async fn agent_budget_status(
    ...
    let usage_store = synthreal_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => synthreal_types::agent::SessionId(u),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = synthreal_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn find_session_by_label(
    ...
    let agent_id = match agent_id_str.parse::<uuid::Uuid>() {
        Ok(u) => synthreal_types::agent::AgentId(u),
        Err(_) => {
            // Try name lookup
...
pub async fn migrate_detect() -> impl IntoResponse {
    match synthreal_migrate::openclaw::detect_openclaw_home() {
        Some(path) => {
            let scan = synthreal_migrate::openclaw::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "openclaw" => synthreal_migrate::MigrateSource::OpenClaw,
        "langchain" => synthreal_migrate::MigrateSource::LangChain,
        "autogpt" => synthreal_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
    ...
    let models: Vec<serde_json::Value> = catalog
        ...
        .filter(|m| {
            ...
            if available_only {
                ...
                if let Some(p) = provider {
                    if p.auth_status == synthreal_types::model_catalog::AuthStatus::Missing {
                        return false;
                    }
                }
            }
        ...
        .map(|m| {
            ...
            let available = catalog
                ...
                .map(|p| p.auth_status != synthreal_types::model_catalog::AuthStatus::Missing)
                .unwrap_or(m.tier == synthreal_types::model_catalog::ModelTier::Custom);
            serde_json::json!({
                "id": m.id,
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let provider_list: Vec<synthreal_types::model_catalog::ProviderInfo> = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
    ...
    let probe_futures: Vec<_> = local_providers
        .iter()
        .map(|(_, id, url)| {
            synthreal_runtime::provider_health::probe_provider_cached(id, url, cache)
        })
        .collect();
    ...
    let mut probe_map: HashMap<usize, synthreal_runtime::provider_health::ProbeResult> =
...
///
/// Persists to `~/.synthreal/custom_models.json` and makes the model immediately
...
pub async fn add_custom_model(
    ...
    let entry = synthreal_types::model_catalog::ModelCatalogEntry {
        id: id.clone(),
        display_name: display,
        provider: provider.clone(),
        tier: synthreal_types::model_catalog::ModelTier::Custom,
        context_window,
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = synthreal_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
        ...
        let card = serde_json::json!({
            "name": "synthreal",
            "description": "Synthreal Agent OS — no agents spawned yet",
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = synthreal_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = synthreal_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: synthreal_runtime::a2a::A2aTaskStatus::Working.into(),
        messages: vec![synthreal_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![synthreal_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = synthreal_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = synthreal_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![synthreal_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = synthreal_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    let mut tools = builtin_tool_definitions();
    {
        ...
        for skill_tool in registry.all_tool_definitions() {
            tools.push(synthreal_types::tool::ToolDefinition {
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn synthreal_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn synthreal_runtime::kernel_handle::KernelHandle>;
        let result = synthreal_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = synthreal_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => synthreal_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = synthreal_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.synthreal/secrets.env`, sets env var in process,
...
pub async fn set_provider_key(
    ...
    let switched = if !current_has_key && current_provider != name {
        ...
        if let Some(model_id) = default_model {
            ...
            {
                let new_dm = synthreal_types::config::DefaultModelConfig {
                    provider: name.clone(),
                    model: model_id,
        ...
        if needs_update {
            ...
            *guard = Some(synthreal_types::config::DefaultModelConfig {
                api_key_env: env_var.clone(),
                ..base
            });
        }
...
pub async fn test_provider(
    ...
    let driver_config = synthreal_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match synthreal_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
            // Send a minimal completion request to test connectivity
            let test_req = synthreal_runtime::llm_driver::CompletionRequest {
                model: default_model.clone(),
                messages: vec![synthreal_types::message::Message::user("Hi")],
                tools: vec![],
                max_tokens: 1,
                temperature: 0.0,
                system: None,
...
pub async fn set_provider_url(
    ...
    let probe = synthreal_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.synthreal/skills/{name}/
...
fn load_skill_declared_config(
    ...
    skill_name: &str,
) -> Option<std::collections::HashMap<String, synthreal_skills::config_injection::SkillConfigVar>> {
    ...
    if skill_dir.is_dir() {
        ...
        if toml_path.exists() {
            if let Ok(text) = std::fs::read_to_string(&toml_path) {
                if let Ok(manifest) = toml::from_str::<synthreal_skills::SkillManifest>(&text) {
        ...
        if skillmd_path.exists() {
            if let Ok(text) = std::fs::read_to_string(&skillmd_path) {
                if let Ok(converted) =
                    synthreal_skills::openclaw_compat::convert_skillmd_str(skill_name, &text)
                {
    ...
    for (bundled_name, content) in synthreal_skills::bundled::bundled_skills() {
        if bundled_name == skill_name {
            if let Ok(converted) =
                synthreal_skills::openclaw_compat::convert_skillmd_str(bundled_name, content)
            {
                return Some(converted.config_vars);
            }
        }
...
/// [`synthreal_skills::config_injection::is_secret_name`]).
fn build_skill_config_snapshot(
    state: &Arc<AppState>,
    skill_name: &str,
) -> Option<serde_json::Value> {
    use synthreal_skills::config_injection::is_secret_name;
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(synthreal_extensions::IntegrationStatus::Ready) => "ready",
                Some(synthreal_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = synthreal_extensions::InstalledIntegration {
                id: id.clone(),
...
// shared-memory key (`__synthreal_schedules`) that no executor ever read — so
...
fn cron_job_to_schedule_view(
    ...
    use synthreal_types::scheduler::{CronAction, CronSchedule};
...
pub async fn create_schedule(
    ...
    if let Some(arr) = delivery_targets_raw.as_array() {
        for (idx, t) in arr.iter().enumerate() {
            if let Err(e) =
                serde_json::from_value::<synthreal_types::scheduler::CronDeliveryTarget>(t.clone())
            {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({
        ...
        Ok(resp) => {
            ...
            if !enabled {
                if let Ok(uuid) = uuid::Uuid::parse_str(&job_id) {
                    let cj_id = synthreal_types::scheduler::CronJobId(uuid);
...
pub async fn update_schedule(
    ...
    if let Some(raw_targets) = req.get("delivery_targets") {
        ...
        let mut parsed: Vec<synthreal_types::scheduler::CronDeliveryTarget> =
            Vec::with_capacity(arr.len());
        for (idx, t) in arr.iter().enumerate() {
            match serde_json::from_value::<synthreal_types::scheduler::CronDeliveryTarget>(t.clone())
            {
...
pub async fn run_schedule(
    ...
    let job = match state.kernel.cron_scheduler.try_claim_for_run(cj_id) {
        Ok(j) => j,
        Err(synthreal_kernel::cron::ClaimError::NotFound) => {
            return (
                StatusCode::NOT_FOUND,
                Json(serde_json::json!({"error": "Schedule not found"})),
            );
        }
        Err(synthreal_kernel::cron::ClaimError::Disabled) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub struct PatchAgentConfigRequest {
    ...
    pub fallback_models: Option<Vec<synthreal_types::agent::FallbackModel>>,
}
...
pub async fn upload_file(
    ...
    let transcription = if content_type.starts_with("audio/") {
        let attachment = synthreal_types::media::MediaAttachment {
            media_type: synthreal_types::media::MediaType::Audio,
            mime_type: content_type.clone(),
            source: synthreal_types::media::MediaSource::FilePath {
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("synthreal_uploads").join(&file_id);
...
pub async fn list_approvals(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    approvals.extend(recent.into_iter().map(|record| {
        ...
        let status = match record.decision {
            synthreal_types::approval::ApprovalDecision::Approved => "approved",
            synthreal_types::approval::ApprovalDecision::Denied => "rejected",
            synthreal_types::approval::ApprovalDecision::TimedOut => "expired",
        };
        serde_json::json!({
            "id": request.id,
...
pub async fn create_approval(
    ...
    use synthreal_types::approval::{ApprovalRequest, RiskLevel};
...
pub async fn approve_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        synthreal_types::approval::ApprovalDecision::Approved,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn reject_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        synthreal_types::approval::ApprovalDecision::Denied,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn delete_cron_job(
    ...
    match uuid::Uuid::parse_str(&id) {
        Ok(uuid) => {
            let job_id = synthreal_types::scheduler::CronJobId(uuid);
...
pub async fn webhook_wake(
    ...
    Json(body): Json<synthreal_types::webhook::WakePayload>,
) -> impl IntoResponse {
...
pub async fn webhook_agent(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<synthreal_types::webhook::AgentHookPayload>,
) -> impl IntoResponse {
    // Check if webhook triggers are enabled
    let wh_config = match &state.kernel.config.webhook_triggers {
        Some(c) if c.enabled => c,
...
pub async fn add_binding(
    State(state): State<Arc<AppState>>,
    Json(binding): Json<synthreal_types::config::AgentBinding>,
) -> impl IntoResponse {
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("synthreal://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = synthreal_kernel::pairing::PairedDevice {
...
pub async fn list_commands(Query(params): Query<CommandsQuery>) -> impl IntoResponse {
    use synthreal_types::commands::{self, CommandCategory, Surfaces};
...
pub async fn copilot_oauth_start() -> impl IntoResponse {
    ...
    match synthreal_runtime::copilot_oauth::start_device_flow().await {
        Ok(resp) => {
...
pub async fn copilot_oauth_poll(
    ...
    match synthreal_runtime::copilot_oauth::poll_device_flow(&device_code).await {
        synthreal_runtime::copilot_oauth::DeviceFlowStatus::Pending => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "pending"})),
        ),
        synthreal_runtime::copilot_oauth::DeviceFlowStatus::Complete { access_token } => {
        ...
        synthreal_runtime::copilot_oauth::DeviceFlowStatus::SlowDown { new_interval } => {
            // Update interval
        ...
        synthreal_runtime::copilot_oauth::DeviceFlowStatus::Expired => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "expired"})),
        ...
        synthreal_runtime::copilot_oauth::DeviceFlowStatus::AccessDenied => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
        ...
        synthreal_runtime::copilot_oauth::DeviceFlowStatus::Error(e) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "error", "error": e})),
        ),
    }
...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use synthreal_types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let synthreal_types::event::EventPayload::Message(_) = &event.payload {
            if let synthreal_types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    event: &synthreal_types::event::Event,
    agents: &[synthreal_types::agent::AgentEntry],
) -> Option<synthreal_types::comms::CommsEvent> {
    use synthreal_types::comms::{CommsEvent, CommsEventKind};
    use synthreal_types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: synthreal_types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &synthreal_runtime::audit::AuditEntry,
    agents: &[synthreal_types::agent::AgentEntry],
) -> Option<synthreal_types::comms::CommsEvent> {
    use synthreal_types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    synthreal_types::truncate_str(id, 12).to_string()
                }
            })
    };
    ...
    let (kind, detail, target_label) = match action_str.as_str() {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!(
                        ...
                        in_tok,
                        out_tok,
                        synthreal_types::truncate_str(&entry.outcome, 80)
                    )
                }
            } else if entry.outcome != "ok" {
                format!(
                    "{} — {}",
                    synthreal_types::truncate_str(&entry.detail, 80),
                    synthreal_types::truncate_str(&entry.outcome, 80)
                )
            } else {
                synthreal_types::truncate_str(&entry.detail, 200).to_string()
            };
        ...
        "AgentSpawn" => (
            ...
            format!(
                "Agent spawned: {}",
                synthreal_types::truncate_str(&entry.detail, 100)
            ),
            "",
        ),
        "AgentKill" => (
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<synthreal_types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_send(
    State(state): State<Arc<AppState>>,
    Json(req): Json<synthreal_types::comms::CommsSendRequest>,
) -> impl IntoResponse {
    // Validate from agent exists
    let from_id: synthreal_types::agent::AgentId = match req.from_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let to_id: synthreal_types::agent::AgentId = match req.to_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<synthreal_types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
...
pub async fn auth_login(
    ...
    let cookie =
        format!("synthreal_session={token}; Path=/; HttpOnly; SameSite=Strict; Max-Age={ttl_secs}");
...
pub async fn auth_logout() -> impl IntoResponse {
    let cookie = "synthreal_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0";
    (
        StatusCode::OK,
...
mod channel_config_tests {
    ...
    fn test_is_channel_configured_wecom_none() {
        let config = synthreal_types::config::ChannelsConfig::default();
        assert!(!is_channel_configured(&config, "wecom"));
    }

    #[test]
    fn test_is_channel_configured_wecom_some() {
        let mut config = synthreal_types::config::ChannelsConfig::default();
        config.wecom = Some(synthreal_types::config::WeComConfig {
            ...
            overrides: synthreal_types::config::ChannelOverrides::default(),
        });
        assert!(is_channel_configured(&config, "wecom"));
    }
