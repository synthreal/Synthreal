...
/// Extract the `synthreal_session` cookie value from a `Cookie` header string.
///
    ...
    headers
        ...
        .and_then(|cookies| {
            cookies.split(';').find_map(|c| {
                c.trim()
                    .strip_prefix("synthreal_session=")
                    .map(|v| v.to_string())
            })
        })
}
...
mod tests {
    ...
    fn test_extract_session_cookie_present() {
        ...
        h.insert(
            "cookie",
            "foo=bar; synthreal_session=abc.def.ghi; baz=qux"
                .parse()
                .unwrap(),
        );
    ...
    fn test_extract_session_cookie_only_value() {
        let mut h = axum::http::HeaderMap::new();
        h.insert("cookie", "synthreal_session=lonely".parse().unwrap());
