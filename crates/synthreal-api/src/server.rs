//! Synthreal daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.synthreal/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. synthreal-desktop)
...
pub async fn build_router(
    ...
    let state = Arc::new(AppState {
        ...
        provider_probe_cache: synthreal_runtime::provider_health::ProbeCache::new(),
    ...
    if state.kernel.config.auth.enabled && !ph.is_empty() && !ph.starts_with("$argon2") {
        tracing::warn!(
            "Dashboard auth password_hash is not in Argon2id format. \
             Login will fail. Regenerate with: synthreal auth hash-password"
        );
    }
    ...
    if api_key.is_empty() && !state.kernel.config.auth.enabled && !bind_is_loopback {
        if allow_no_auth {
            tracing::warn!(
                "SYNTHREAL_ALLOW_NO_AUTH=1 is set. Running WITHOUT authentication on {}. \
            ...
            tracing::warn!(
                "No api_key configured and server is bound to {} (non-loopback). \
                 ...
                 Set SYNTHREAL_API_KEY (or api_key in config.toml), or bind to 127.0.0.1, \
                 or set SYNTHREAL_ALLOW_NO_AUTH=1 to explicitly run open.",
                listen_addr
            );
        }
    }
...
/// Start the Synthreal daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("Synthreal API server listening on http://{addr}");
...
/// Check if an Synthreal daemon is actually responding at the given address.
