//! Production middleware for the Synthreal API server.
//!
//! Provides:
...
pub struct AuthState {
    ...
    /// Set from `SYNTHREAL_ALLOW_NO_AUTH=1` to permit running without an api_key
...
/// `SYNTHREAL_ALLOW_NO_AUTH=1`; this opts out of fail-closed and is
...
pub async fn auth(
    ...
    // behavior requires setting `SYNTHREAL_ALLOW_NO_AUTH=1`, which is logged
    // loudly at startup.
    //
    ...
    if api_key_trimmed.is_empty() && !auth_state.auth_enabled {
        ...
        return Response::builder()
            ...
            .body(Body::from(
                serde_json::json!({
                    "error": "API key required for non-loopback requests. Set SYNTHREAL_API_KEY or bind to 127.0.0.1."
                })
                .to_string(),
            ))
            .unwrap_or_default();
