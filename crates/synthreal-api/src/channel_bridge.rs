//! Channel bridge wiring — connects the Synthreal kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `SynthrealKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use synthreal_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use synthreal_channels::discord::DiscordAdapter;
use synthreal_channels::email::EmailAdapter;
use synthreal_channels::google_chat::GoogleChatAdapter;
use synthreal_channels::irc::IrcAdapter;
use synthreal_channels::matrix::MatrixAdapter;
use synthreal_channels::mattermost::MattermostAdapter;
use synthreal_channels::rocketchat::RocketChatAdapter;
use synthreal_channels::router::AgentRouter;
use synthreal_channels::signal::SignalAdapter;
use synthreal_channels::slack::SlackAdapter;
use synthreal_channels::teams::TeamsAdapter;
use synthreal_channels::telegram::TelegramAdapter;
use synthreal_channels::twitch::TwitchAdapter;
use synthreal_channels::types::ChannelAdapter;
use synthreal_channels::whatsapp::WhatsAppAdapter;
use synthreal_channels::xmpp::XmppAdapter;
use synthreal_channels::zulip::ZulipAdapter;
// Wave 3
use synthreal_channels::bluesky::BlueskyAdapter;
use synthreal_channels::feishu::FeishuAdapter;
use synthreal_channels::line::LineAdapter;
use synthreal_channels::mastodon::MastodonAdapter;
use synthreal_channels::messenger::MessengerAdapter;
use synthreal_channels::reddit::RedditAdapter;
use synthreal_channels::revolt::RevoltAdapter;
use synthreal_channels::viber::ViberAdapter;
use synthreal_types::config::FeishuMode;
// Wave 4
use synthreal_channels::flock::FlockAdapter;
use synthreal_channels::guilded::GuildedAdapter;
use synthreal_channels::keybase::KeybaseAdapter;
use synthreal_channels::nextcloud::NextcloudAdapter;
use synthreal_channels::nostr::NostrAdapter;
use synthreal_channels::pumble::PumbleAdapter;
use synthreal_channels::threema::ThreemaAdapter;
use synthreal_channels::twist::TwistAdapter;
use synthreal_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use synthreal_channels::dingtalk::DingTalkAdapter;
use synthreal_channels::dingtalk_stream::DingTalkStreamAdapter;
use synthreal_channels::discourse::DiscourseAdapter;
use synthreal_channels::gitter::GitterAdapter;
use synthreal_channels::gotify::GotifyAdapter;
use synthreal_channels::linkedin::LinkedInAdapter;
use synthreal_channels::mqtt::MqttAdapter;
use synthreal_channels::mumble::MumbleAdapter;
use synthreal_channels::ntfy::NtfyAdapter;
use synthreal_channels::webhook::WebhookAdapter;
use synthreal_channels::wecom::WeComAdapter;
use synthreal_kernel::SynthrealKernel;
use synthreal_runtime::kernel_handle::KernelHandle;
use synthreal_types::agent::AgentId;
use std::sync::Arc;
use std::time::{Duration, Instant};
use tracing::{error, info, warn};

use synthreal_runtime::str_utils::safe_truncate_str;

/// Wraps `SynthrealKernel` to implement `ChannelBridgeHandle`.
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn send_message_with_blocks(
        ...
        let text: String = blocks
            ...
            .filter_map(|b| match b {
                synthreal_types::message::ContentBlock::Text { text, .. } => Some(text.as_str()),
                _ => None,
            })
            .collect::<Vec<_>>()
            .join("\n");
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.synthreal/agents/{name}/agent.toml
        ...
        let manifest: synthreal_types::agent::AgentManifest =
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "Synthreal status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "Synthreal status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_providers_text(&self) -> String {
        ...
        for p in catalog.list_providers() {
            let status = match p.auth_status {
                synthreal_types::model_catalog::AuthStatus::Configured => "configured",
                synthreal_types::model_catalog::AuthStatus::Missing => "not configured",
                synthreal_types::model_catalog::AuthStatus::NotRequired => "local (no key needed)",
            };
            msg.push_str(&format!(
                "  {} — {} [{}, {} model(s)]\n",
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.synthreal/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    synthreal_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    synthreal_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                synthreal_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                synthreal_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                synthreal_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn manage_schedule_text(&self, action: &str, args: &[String]) -> String {
        match action {
            "add" => {
                ...
                let job = synthreal_types::scheduler::CronJob {
                    id: synthreal_types::scheduler::CronJobId::new(),
                    agent_id: agent.id,
                    name: format!("chat-{}", &agent.name),
                    enabled: true,
                    schedule: synthreal_types::scheduler::CronSchedule::Cron {
                        expr: cron_expr.clone(),
                        tz: None,
                    },
                    action: synthreal_types::scheduler::CronAction::AgentTurn {
                        message: message.clone(),
                        model_override: None,
                        timeout_secs: None,
                    },
                    delivery: synthreal_types::scheduler::CronDelivery::None,
                ...
                match matched.len() {
                    ...
                    1 => {
                        ...
                        let message = match &j.action {
                            synthreal_types::scheduler::CronAction::AgentTurn {
                                message, ..
                            } => message.clone(),
                            synthreal_types::scheduler::CronAction::SystemEvent { text } => {
                                text.clone()
                            }
                            synthreal_types::scheduler::CronAction::WorkflowRun {
                                workflow_id,
                                input,
                                ..
                            } => {
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<synthreal_types::config::ChannelOverrides> {
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => synthreal_kernel::auth::Action::ChatWithAgent,
            "spawn" => synthreal_kernel::auth::Action::SpawnAgent,
            "kill" => synthreal_kernel::auth::Action::KillAgent,
            "install_skill" => synthreal_kernel::auth::Action::InstallSkill,
            _ => synthreal_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            synthreal_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            synthreal_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn send_channel_message(
        ...
        <SynthrealKernel as KernelHandle>::send_channel_message(
            &self.kernel,
            channel_type,
            recipient,
            message,
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = synthreal_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<synthreal_kernel::triggers::TriggerPattern> {
    use synthreal_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<SynthrealKernel>) -> Option<BridgeManager> {
...
pub async fn start_channel_bridge_with_config(
    ...
    if let Some(ref fs_config) = config.feishu {
        if let Some(secret) = read_token(&fs_config.app_secret_env, "Feishu") {
            let region = synthreal_channels::feishu::FeishuRegion::parse_region(&fs_config.region);
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = synthreal_kernel::config::load_config(Some(&config_path));
...
mod tests {
    ...
    fn test_feishu_bridge_mode_defaults_to_websocket() {
        let config: synthreal_types::config::KernelConfig = toml::from_str(
            r#"
            [channels.feishu]
            app_id = "cli_test"
        ...
        assert_eq!(feishu.mode, synthreal_types::config::FeishuMode::Websocket);
    }
