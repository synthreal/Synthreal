...
use synthreal_runtime::llm_errors;
use synthreal_types::agent::AgentId;
use synthreal_types::commands::{self, Surfaces};
...
pub(crate) struct WsAuthCtx<'a> {
    ...
    /// True iff `SYNTHREAL_ALLOW_NO_AUTH=1` is set (loose mode for LAN binds).
...
///   3. `synthreal_session=<token>` cookie when dashboard auth is enabled
///   4. Loopback origin when no api_key is configured
///   5. Any origin when `SYNTHREAL_ALLOW_NO_AUTH=1`
///
...
pub(crate) fn check_ws_auth(ctx: &WsAuthCtx<'_>) -> Result<(), axum::http::StatusCode> {
    ...
    if ctx.api_key.is_empty() {
        ...
        // path (loopback or explicit SYNTHREAL_ALLOW_NO_AUTH=1).
...
/// set custom headers), or the `synthreal_session` cookie set by the
...
pub async fn agent_ws(
    ...
    let allow_no_auth = std::env::var("SYNTHREAL_ALLOW_NO_AUTH")
    ...
    if let Err(status) = check_ws_auth(&auth_ctx) {
        warn!(
            ip = %addr.ip(),
            "WebSocket upgrade rejected: no valid Bearer token, ?token=, or synthreal_session cookie"
        );
...
async fn handle_text_message(
    ...
    match msg_type {
        "message" => {
            ...
            let mut ws_content_blocks: Option<Vec<synthreal_types::message::ContentBlock>> = None;
            ...
            match state.kernel.send_message_streaming(
                ...
                Ok((mut rx, handle)) => {
                    ...
                    let stream_task = tokio::spawn(async move {
                        ...
                        let mut stream_usage: Option<synthreal_types::message::TokenUsage> = None;
...
async fn handle_command(
    ...
    match canonical {
        ...
        "context" => match state.kernel.context_report(agent_id) {
            Ok(report) => {
                let formatted = synthreal_runtime::compactor::format_context_report(&report);
                serde_json::json!({
                    "type": "command_result",
                    "command": cmd,
                    "message": formatted,
...
///
/// Uses the proper LLM error classifier from `synthreal_runtime::llm_errors`
/// for comprehensive 20-provider coverage with actionable advice.
fn classify_streaming_error(err: &synthreal_kernel::error::KernelError) -> String {
...
pub fn start_ws_cron_broadcaster(kernel: Arc<SynthrealKernel>) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            match event {
                Ok(event) => {
                    if let synthreal_types::event::EventPayload::System(
                        synthreal_types::event::SystemEvent::CronJobExecuted {
                            agent_id,
                            job_id,
                            job_name,
                            trigger_message,
...
mod tests {
    ...
    fn ws_auth_accepts_session_cookie() {
        ...
        let cookie = format!("foo=bar; synthreal_session={token}");
        let mut headers = axum::http::HeaderMap::new();
    ...
    fn ws_auth_session_cookie_rejected_when_auth_disabled() {
        ...
        headers.insert(
            "cookie",
            format!("synthreal_session={token}").parse().unwrap(),
        );
        let uri = empty_uri();
        let ctx = WsAuthCtx {
            api_key: secret,
    ...
    fn ws_auth_rejects_wrong_session_cookie() {
        ...
        let mut headers = axum::http::HeaderMap::new();
        headers.insert("cookie", format!("synthreal_session={bad}").parse().unwrap());
        let uri = empty_uri();
        let ctx = WsAuthCtx {
            api_key: "secret",
            auth_enabled: true,
