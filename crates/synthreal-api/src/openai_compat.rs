...
//!
//! Allows any OpenAI-compatible client library to talk to Synthreal agents.
//! The `model` field resolves to an agent (by name, UUID, or `synthreal:<name>`),
...
use synthreal_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "synthreal:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("synthreal:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("synthreal:{}", e.name),
            object: "model",
            created,
            owned_by: "synthreal".to_string(),
        })
        .collect();
