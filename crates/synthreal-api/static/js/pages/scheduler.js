// Synthreal Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadChannelTypes() {
      try {
        var data = await SynthrealAPI.get('/api/channels');
    ...
    async loadJobs() {
      var data = await SynthrealAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await SynthrealAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        SynthrealToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        SynthrealToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await SynthrealAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true, delivery_targets: [] };
        SynthrealToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        SynthrealToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await SynthrealAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        SynthrealToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        SynthrealToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      SynthrealToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await SynthrealAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          SynthrealToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          SynthrealToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await SynthrealAPI.post('/api/cron/jobs/' + job.id + '/run', {});
        if (result.status === 'triggered' || result.status === 'completed') {
          SynthrealToast.success('Job "' + (job.name || 'job') + '" triggered');
          ...
          SynthrealToast.error('Run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        SynthrealToast.error('Run failed: ' + (e.message || e));
      }
      this.runningJobId = '';
    },
    ...
    async toggleExpand(job) {
      ...
      try {
        var data = await SynthrealAPI.get('/api/schedules/' + job.id + '/delivery-log');
        this.deliveryLog = {
    ...
    async saveEditTargets() {
      ...
      try {
        ...
        await SynthrealAPI.put('/api/schedules/' + this.editingTargetsJobId, {
          delivery_targets: clean
        });
        SynthrealToast.success('Delivery targets updated');
        this.cancelEditTargets();
        await this.loadJobs();
      } catch(e) {
        SynthrealToast.error('Failed to update targets: ' + (e.message || e));
      }
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await SynthrealAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        SynthrealToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        SynthrealToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      SynthrealToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await SynthrealAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          SynthrealToast.success('Trigger deleted');
        } catch(e) {
          SynthrealToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
