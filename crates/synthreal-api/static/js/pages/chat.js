// Synthreal Chat Page — Agent chat with markdown + streaming
'use strict';
  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            SynthrealAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      SynthrealAPI.get('/api/models').then(function(data) {
        ...
        SynthrealToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      var t = typeof window.t === 'function' ? window.t : function(s) { return s; };
      SynthrealAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function(resp) {
        ...
        SynthrealToast.success(t('chat.model_switched') + ' ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        SynthrealToast.error(t('chat.model_switch_failed') + ': ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      SynthrealAPI.get('/api/commands?surface=web').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            SynthrealAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              SynthrealToast.success('Session reset');
            }).catch(function(e) { SynthrealToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            SynthrealAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { SynthrealToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            SynthrealAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { SynthrealToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && SynthrealAPI.isWsConnected()) {
            SynthrealAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected (' + (SynthrealAPI.getConnectionState ? SynthrealAPI.getConnectionState() : 'unknown') + '). Pick an agent or check that your session is still valid.', meta: '', tools: [] });
            self.scrollToBottom();
          }
          break;
        case '/verbose':
          if (self.currentAgent && SynthrealAPI.isWsConnected()) {
            SynthrealAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected (' + (SynthrealAPI.getConnectionState ? SynthrealAPI.getConnectionState() : 'unknown') + '). Pick an agent or check that your session is still valid.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && SynthrealAPI.isWsConnected()) {
            SynthrealAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected (' + (SynthrealAPI.getConnectionState ? SynthrealAPI.getConnectionState() : 'unknown') + ').', meta: '', tools: [] });
            self.scrollToBottom();
          }
          break;
        case '/status':
          SynthrealAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              SynthrealAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function(resp) {
                ...
                self.scrollToBottom();
              }).catch(function(e) { SynthrealToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          SynthrealAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          SynthrealAPI.get('/api/network/status').then(function(ns) {
          ...
          break;
        case '/a2a':
          SynthrealAPI.get('/api/a2a/agents').then(function(res) {
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await SynthrealAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await SynthrealAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await SynthrealAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof SynthrealToast !== 'undefined') SynthrealToast.success(t('chat.session_created'));
      } catch(e) {
        if (typeof SynthrealToast !== 'undefined') SynthrealToast.error(t('chat.session_create_failed'));
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await SynthrealAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof SynthrealToast !== 'undefined') SynthrealToast.error('Failed to switch session');
      }
    },
    ...
    connectWs(agentId) {
      ...
      SynthrealAPI.wsConnect(agentId, {
        onOpen: function() {
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await SynthrealAPI.upload(this.currentAgent.id, att.file);
            ...
            SynthrealToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      if (SynthrealAPI.wsSend(wsPayload)) {
      ...
      // HTTP fallback
      var t = typeof window.t === 'function' ? window.t : function(s) { return s; };
      if (!SynthrealAPI.isWsConnected()) {
        SynthrealToast.info(t('chat.using_http_mode'));
      }
      ...
      try {
        ...
        var res = await SynthrealAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      SynthrealAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      SynthrealToast.confirm(t('chat.stop_agent_title'), t('chat.stop_agent_confirm') + ' "' + name + '"?', async function() {
        try {
          await SynthrealAPI.del('/api/agents/' + self.currentAgent.id);
          SynthrealAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          try { localStorage.removeItem('of-active-agent'); } catch(e) { /* ignore */ }
          SynthrealToast.success(t('chat.agent_stopped') + ' "' + name + '"');
          Alpine.store('app').refreshAgents();
        } catch(e) {
          SynthrealToast.error(t('chat.stop_agent_failed') + ': ' + e.message);
        }
      });
    },

    // Permanently uninstall the agent: kill + remove ~/.synthreal/agents/<name>/
    // Issue #1163.
    uninstallAgent: function() {
      ...
      SynthrealToast.confirm(
        ...
        async function() {
          try {
            var res = await SynthrealAPI.del('/api/agents/' + agentId + '/uninstall');
            SynthrealAPI.wsDisconnect();
            self._wsAgent = null;
            self.currentAgent = null;
            self.messages = [];
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          SynthrealToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          SynthrealToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof SynthrealToast !== 'undefined') SynthrealToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await SynthrealAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof SynthrealToast !== 'undefined') SynthrealToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
