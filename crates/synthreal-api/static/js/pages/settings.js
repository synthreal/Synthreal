// Synthreal Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
  ...
  return {
    ...
    configurableFeatures: [
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.synthreal/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await SynthrealAPI.get('/api/version');
        var status = await SynthrealAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await SynthrealAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await SynthrealAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await SynthrealAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await SynthrealAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await SynthrealAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await SynthrealAPI.post('/api/models/custom', {
          id: id,
    ...
    async deleteCustomModel(modelId) {
      ...
      try {
        await SynthrealAPI.del('/api/models/custom/' + encodeURIComponent(modelId));
        SynthrealToast.success('Model deleted');
        await this.loadModels();
      } catch(e) {
        SynthrealToast.error('Failed to delete: ' + (e.message || 'Unknown error'));
      }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          SynthrealAPI.get('/api/config/schema').catch(function() { return {}; }),
          SynthrealAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await SynthrealAPI.post('/api/config/set', { path: path, value: value });
        this.configDirty[key] = false;
        SynthrealToast.success('Saved ' + field);
      } catch(e) {
        SynthrealToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { SynthrealToast.error('Please enter an API key'); return; }
      try {
        var resp = await SynthrealAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        if (resp && resp.switched_default) {
          SynthrealToast.warning(resp.message || 'Default provider was switched to ' + provider.display_name);
        } else {
          SynthrealToast.success('API key saved for ' + provider.display_name);
        }
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        SynthrealToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await SynthrealAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        SynthrealToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        SynthrealToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await SynthrealAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        SynthrealToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await SynthrealAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            SynthrealToast.success('GitHub Copilot authenticated successfully!');
            ...
            SynthrealToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            SynthrealToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            SynthrealToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          SynthrealToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await SynthrealAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          SynthrealToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          SynthrealToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        SynthrealToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { SynthrealToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        SynthrealToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await SynthrealAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          SynthrealToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          SynthrealToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        SynthrealToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async addCustomProvider() {
      ...
      if (!name) { SynthrealToast.error('Please enter a provider name'); return; }
      var url = this.customProviderUrl.trim();
      if (!url) { SynthrealToast.error('Please enter a base URL'); return; }
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        SynthrealToast.error('URL must start with http:// or https://'); return;
      }
      ...
      try {
        var result = await SynthrealAPI.put('/api/providers/' + encodeURIComponent(name) + '/url', { base_url: url });
        if (this.customProviderKey.trim()) {
          await SynthrealAPI.post('/api/providers/' + encodeURIComponent(name) + '/key', { key: this.customProviderKey.trim() });
        }
        this.customProviderName = '';
        this.customProviderUrl = '';
        this.customProviderKey = '';
        this.customProviderStatus = '';
        SynthrealToast.success('Provider "' + name + '" added' + (result.reachable ? ' (reachable)' : ' (not reachable yet)'));
        await this.loadProviders();
      } catch(e) {
        this.customProviderStatus = 'Error: ' + (e.message || 'Failed');
        SynthrealToast.error('Failed to add provider: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await SynthrealAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await SynthrealAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await SynthrealAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await SynthrealAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await SynthrealAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          SynthrealToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        SynthrealToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await SynthrealAPI.post('/api/migrate', {
          source: 'openclaw',
