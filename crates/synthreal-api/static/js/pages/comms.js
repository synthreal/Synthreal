// Synthreal Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          SynthrealAPI.get('/api/comms/topology'),
          SynthrealAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      if (SynthrealAPI.apiKey) url += '?token=' + encodeURIComponent(SynthrealAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await SynthrealAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await SynthrealAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        SynthrealToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        SynthrealToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await SynthrealAPI.post('/api/comms/task', body);
        SynthrealToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        SynthrealToast.error(e.message || 'Task failed');
      }
