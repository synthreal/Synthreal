// Synthreal Agents Page — Multi-step spawn wizard, detail view with tabs, file editor, personality presets
'use strict';
...
function agentsPage() {
  return {
    ...
    spawnStep: 1,
    spawnIdentity: { emoji: '', color: '#7C3AED', archetype: '' },
    selectedPreset: '',
    soulContent: '',
    emojiOptions: [
    ...
    async loadSpawnProfiles() {
      ...
      try {
        var data = await SynthrealAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          SynthrealAPI.get('/api/templates'),
          SynthrealAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    async showDetail(agent) {
      ...
      var detail = agent;
      try {
        var full = await SynthrealAPI.get('/api/agents/' + agent.id);
    ...
    killAgent(agent) {
      var self = this;
      SynthrealToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await SynthrealAPI.del('/api/agents/' + agent.id);
          SynthrealToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          SynthrealToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },

    // Issue #1163: uninstall an agent (kill + remove ~/.synthreal/agents/<name>/).
    uninstallAgent(agent) {
      var self = this;
      SynthrealToast.confirm(
        'Uninstall Agent',
        'Uninstall agent "' + agent.name + '"? This stops the agent AND deletes its files from your workspace. This cannot be undone.',
        async function() {
          try {
            var res = await SynthrealAPI.del('/api/agents/' + agent.id + '/uninstall');
            var msg = 'Agent "' + agent.name + '" uninstalled';
            if (res && res.dir_removed === false) {
              msg += ' (no on-disk files found)';
            }
            SynthrealToast.success(msg);
            self.showDetailModal = false;
            await Alpine.store('app').refreshAgents();
          } catch(e) {
            SynthrealToast.error('Failed to uninstall agent: ' + e.message);
          }
        }
      );
    },
    ...
    killAllAgents() {
      ...
      SynthrealToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await SynthrealAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          SynthrealToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          SynthrealToast.success(list.length + ' agent(s) stopped');
        }
      });
    },
    ...
    async openSpawnWizard() {
      ...
      this.spawnIdentity = { emoji: '', color: '#7C3AED', archetype: '' };
      this.selectedPreset = '';
      this.soulContent = '';
      this.spawnForm.name = '';
      ...
      try {
        var results = await Promise.all([
          SynthrealAPI.get('/api/status').catch(function() { return {}; }),
          SynthrealAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    nextStep() {
      if (this.spawnStep === 1 && !this.spawnForm.name.trim()) {
        SynthrealToast.warn('Please enter an agent name');
        return;
      }
    ...
    async setMode(agent, mode) {
      try {
        await SynthrealAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        SynthrealToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        SynthrealToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        SynthrealToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await SynthrealAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            SynthrealAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            SynthrealAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          SynthrealToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          SynthrealToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        SynthrealToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await SynthrealAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        SynthrealToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await SynthrealAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        SynthrealToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await SynthrealAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        SynthrealToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        SynthrealToast.error('Failed to save file: ' + e.message);
      }
    ...
    async saveConfig() {
      ...
      try {
        await SynthrealAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        SynthrealToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        SynthrealToast.error('Failed to save config: ' + e.message);
      }
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await SynthrealAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          SynthrealToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        SynthrealToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(template) {
      try {
        ...
        if (!manifestToml) {
          ...
          var data = await SynthrealAPI.get('/api/templates/' + encodeURIComponent(template.name));
          manifestToml = data.manifest_toml;
        }
        if (manifestToml) {
          var res = await SynthrealAPI.post('/api/agents', { manifest_toml: manifestToml });
          if (res.agent_id) {
            SynthrealToast.success('Agent "' + (res.name || template.name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || template.name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        SynthrealToast.error('Failed to spawn from template: ' + e.message);
      }
    },
    ...
    async clearHistory(agent) {
      var self = this;
      SynthrealToast.confirm('Clear History', 'Clear all conversation history for "' + agent.name + '"? This cannot be undone.', async function() {
        try {
          await SynthrealAPI.del('/api/agents/' + agent.id + '/history');
          SynthrealToast.success('History cleared for "' + agent.name + '"');
        } catch(e) {
          SynthrealToast.error('Failed to clear history: ' + e.message);
        }
      });
    },
    ...
    async changeModel() {
      ...
      try {
        var resp = await SynthrealAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: this.newModelValue.trim() });
        var providerInfo = (resp && resp.provider) ? ' (provider: ' + resp.provider + ')' : '';
        SynthrealToast.success('Model changed' + providerInfo + ' (memory reset)');
        ...
        SynthrealToast.error('Failed to change model: ' + e.message);
      }
    ...
    async changeProvider() {
      ...
      try {
        ...
        var resp = await SynthrealAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: combined });
        SynthrealToast.success('Provider changed to ' + (resp && resp.provider ? resp.provider : this.newProviderValue.trim()));
        ...
        SynthrealToast.error('Failed to change provider: ' + e.message);
      }
      this.modelSaving = false;
    },
    ...
    async addFallback() {
      ...
      try {
        await SynthrealAPI.patch('/api/agents/' + this.detailAgent.id + '/config', {
          fallback_models: this.detailAgent._fallbacks
        });
        SynthrealToast.success('Fallback added: ' + provider + '/' + model);
      } catch(e) {
        SynthrealToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async removeFallback(idx) {
      ...
      try {
        ...
        SynthrealToast.success('Fallback removed');
      } catch(e) {
        SynthrealToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async loadToolFilters() {
      ...
      try {
        this.toolFilters = await SynthrealAPI.get('/api/agents/' + this.detailAgent.id + '/tools');
      } catch(e) {
    ...
    async saveToolFilters() {
      if (!this.detailAgent) return;
      try {
        await SynthrealAPI.put('/api/agents/' + this.detailAgent.id + '/tools', this.toolFilters);
      } catch(e) {
        SynthrealToast.error('Failed to update tool filters: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await SynthrealAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          SynthrealToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
