// Synthreal Skills Page — OpenClaw/ClawHub ecosystem + local skills + MCP servers
'use strict';
...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await SynthrealAPI.get('/api/skills');
    ...
    async openSkillConfig(skill) {
      ...
      try {
        var data = await SynthrealAPI.get('/api/skills/' + encodeURIComponent(skill.name) + '/config');
    ...
    async saveSkillConfig() {
      ...
      if (this.hasInvalidConfig()) {
        SynthrealToast.error('Fill in all required variables before saving.');
        return;
      }
      ...
      try {
        await SynthrealAPI.put('/api/skills/' + encodeURIComponent(this.configSkill.name) + '/config', { values: payload });
        SynthrealToast.success('Saved, reloading agents\u2026');
        ...
        SynthrealToast.error('Save failed: ' + (e.message || 'unknown error'));
      }
    ...
    async resetSkillConfigVar(name) {
      ...
      try {
        await SynthrealAPI.del('/api/skills/' + encodeURIComponent(this.configSkill.name) + '/config/' + encodeURIComponent(name));
        SynthrealToast.success('Reset ' + name);
        // Refresh modal state from server.
        var data = await SynthrealAPI.get('/api/skills/' + encodeURIComponent(this.configSkill.name) + '/config');
        this.configResolved = data.resolved || {};
        this.configDraft[name] = '';
      } catch(e) {
        var msg = e.message || 'Reset failed';
        if (msg.indexOf('required') !== -1 || msg.indexOf('409') !== -1) {
          SynthrealToast.error('Cannot reset: ' + decl.description + ' is required with no fallback.');
        } else {
          SynthrealToast.error('Reset failed: ' + msg);
        }
      }
    },
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await SynthrealAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await SynthrealAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await SynthrealAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await SynthrealAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        SynthrealToast.error('Failed to load skill details');
      }
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await SynthrealAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug) + '/code');
        this.skillCode = data.code || '';
        this.skillCodeFilename = data.filename || 'source';
        this.showSkillCode = true;
      } catch(e) {
        SynthrealToast.error('Could not load skill source code');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await SynthrealAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          SynthrealToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          SynthrealToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          SynthrealToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          SynthrealToast.error('Skill blocked by security scan');
        } else {
          SynthrealToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      ...
      SynthrealToast.confirm(
        ...
        async function() {
          try {
            await SynthrealAPI.post('/api/skills/uninstall', { name: name });
            SynthrealToast.success('Skill "' + name + '" uninstalled');
            await self.loadSkills();
          } catch(e) {
            SynthrealToast.error('Failed to uninstall skill: ' + e.message);
          }
        }
      );
    },
    ...
    async createDemoSkill(skill) {
      try {
        await SynthrealAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        SynthrealToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        SynthrealToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await SynthrealAPI.get('/api/mcp/servers');
