// Synthreal Sessions Page — Session listing + Memory tab
'use strict';
...
function sessionsPage() {
  return {
    ...
    async loadSessions() {
      ...
      this.loadError = '';
      try {
        var data = await SynthrealAPI.get('/api/sessions');
    ...
    deleteSession(sessionId) {
      ...
      SynthrealToast.confirm(
        ...
        async function() {
          try {
            await SynthrealAPI.del('/api/sessions/' + sessionId);
            self.sessions = self.sessions.filter(function(s) { return s.session_id !== sessionId; });
            SynthrealToast.success('Session deleted');
          } catch(e) {
            SynthrealToast.error('Failed to delete session: ' + e.message);
          }
        }
      );
    },
    ...
    async loadKv() {
      ...
      this.memLoadError = '';
      try {
        var data = await SynthrealAPI.get('/api/memory/agents/' + this.memAgentId + '/kv');
    ...
    async addKey() {
      ...
      try {
        await SynthrealAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.newKey), { value: value });
        this.showAdd = false;
        SynthrealToast.success('Key "' + this.newKey + '" saved');
        this.newKey = '';
        this.newValue = '""';
        await this.loadKv();
      } catch(e) {
    ...
    deleteKey(key) {
      ...
      SynthrealToast.confirm(
        ...
        async function() {
          try {
            await SynthrealAPI.del('/api/memory/agents/' + self.memAgentId + '/kv/' + encodeURIComponent(key));
            SynthrealToast.success('Key "' + key + '" deleted');
            await self.loadKv();
          } catch(e) {
            SynthrealToast.error('Failed to delete key: ' + e.message);
          }
        }
      );
    },
    ...
    async saveEdit() {
      ...
      try {
        await SynthrealAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.editingKey), { value: value });
        SynthrealToast.success('Key "' + this.editingKey + '" updated');
        this.editingKey = null;
        this.editingValue = '';
        await this.loadKv();
      } catch(e) {
