// Synthreal Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    _chartColors: [
      '#7C3AED', '#3B82F6', '#10B981', '#F59E0B', '#8B5CF6',
    ...
    async loadSummary() {
      try {
        this.summary = await SynthrealAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await SynthrealAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await SynthrealAPI.get('/api/usage/daily');
