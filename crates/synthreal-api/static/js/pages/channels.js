// Synthreal Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async startQR() {
      ...
      try {
        var result = await SynthrealAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          SynthrealToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await SynthrealAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            SynthrealToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await SynthrealAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await SynthrealAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            SynthrealToast.success(this.setupModal.display_name + ' activated!');
          } else {
            SynthrealToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          SynthrealToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
        SynthrealToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async removeChannel() {
      ...
      var self = this;
      SynthrealToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await SynthrealAPI.delete('/api/channels/' + name + '/configure');
          SynthrealToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await SynthrealAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          SynthrealToast.success(result.message);
        } else {
          SynthrealToast.error(result.message);
        }
      } catch(e) {
        SynthrealToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        SynthrealToast.success('Copied to clipboard');
      } catch(e) {
        SynthrealToast.error('Copy failed');
      }
    },
