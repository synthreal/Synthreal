// Synthreal App — Alpine.js init, hash router, global store
'use strict';
...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('synthreal-api-key');
  if (savedKey) SynthrealAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    pendingAgent: null,
    focusMode: localStorage.getItem('synthreal-focus') === 'true',
    ...
    toggleFocusMode() {
      ...
      localStorage.setItem('synthreal-focus', this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await SynthrealAPI.get('/api/agents');
    ...
    async refreshApprovals() {
      try {
        var data = await SynthrealAPI.get('/api/approvals');
        ...
        var signature = pending
          ...
          .sort()
          .join(',');
        if (pending.length > 0 && signature !== this.lastPendingApprovalSignature && typeof SynthrealToast !== 'undefined') {
          SynthrealToast.warn('An agent is waiting for approval. Open Approvals to review.');
        }
    ...
    async checkStatus() {
      try {
        var s = await SynthrealAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[Synthreal] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('synthreal-onboarded')) return;
      try {
        var config = await SynthrealAPI.get('/api/config');
    ...
    async checkAuth() {
      try {
        ...
        var authInfo = await SynthrealAPI.get('/api/auth/check');
      ...
      try {
        await SynthrealAPI.get('/api/tools');
        this.showAuthPrompt = false;
      } catch(e) {
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          var saved = localStorage.getItem('synthreal-api-key');
          if (saved) {
            SynthrealAPI.setAuthToken('');
            localStorage.removeItem('synthreal-api-key');
          }
          this.showAuthPrompt = true;
        }
      }
    ...
    submitApiKey(key) {
      ...
      SynthrealAPI.setAuthToken(key.trim());
      localStorage.setItem('synthreal-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
    ...
    async sessionLogin(username, password) {
      try {
        var result = await SynthrealAPI.post('/api/auth/login', { username: username, password: password });
        if (result.status === 'ok') {
          this.sessionUser = result.username;
          this.showAuthPrompt = false;
          this.refreshAgents();
        } else {
          SynthrealToast.error(result.error || 'Login failed');
        }
      } catch(e) {
        SynthrealToast.error(e.message || 'Login failed');
      }
    },

    async sessionLogout() {
      try {
        await SynthrealAPI.post('/api/auth/logout');
      } catch(e) { /* ignore */ }
      this.sessionUser = null;
      this.showAuthPrompt = true;
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('synthreal-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('synthreal-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('synthreal-sidebar') === 'collapsed',
    ...
    init() {
      ...
      SynthrealAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('synthreal-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('synthreal-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = SynthrealAPI.isWsConnected();
    }
  };
}
