/**
 * Synthreal i18n (Internationalization) Module
 * 
 * Provides runtime language switching for the Synthreal dashboard UI.
...
(function() {
  ...
  function applyTranslations() {
    ...
    if (metaDesc) {
      const desc = t('app.description', { name: 'Synthreal' });
  ...
  async function setLanguage(lang, persist = true) {
    ...
    // Save preference
    if (persist) {
      localStorage.setItem('synthreal_language', lang);
    }
  ...
  async function init() {
    ...
    let lang = localStorage.getItem('synthreal_language');
