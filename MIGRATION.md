# Migrating to Synthreal

This guide covers migrating from OpenClaw (and other frameworks) to Synthreal. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.synthreal/`.

### Options

```bash
# Specify a custom source directory
synthreal migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.synthreal/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
synthreal migrate --from langchain   # Coming soon
synthreal migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (Synthreal) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.synthreal/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.synthreal/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.synthreal/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.synthreal/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.synthreal/config.toml`.
...
```bash
synthreal skill install <skill-name-or-path>
```

Synthreal automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize Synthreal

```bash
synthreal init
```

This creates `~/.synthreal/` with a default `config.toml`.
...
**Synthreal** (`~/.synthreal/config.toml`):
```toml
[default_model]
provider = "anthropic"
model = "claude-sonnet-4-20250514"
...
**Synthreal** (`~/.synthreal/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
**Synthreal** (add to `~/.synthreal/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to Synthreal agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.synthreal/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | Synthreal |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.synthreal/config.toml` |
...
Tools were renamed between OpenClaw and Synthreal for consistency. The migration engine handles this automatically.

| OpenClaw Tool | Synthreal Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | Synthreal Tools |
|------------------|----------------|
...
| OpenClaw Name | Synthreal Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (Synthreal Protocol) peer-to-peer |
...
| **Skills/Plugins** | npm packages | TOML + Python/WASM/Node.js, SynthHub marketplace |
...
```bash
synthreal migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
synthreal skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.synthreal/channels_import.toml
# Copy the [channels.*] sections into ~/.synthreal/config.toml
```
